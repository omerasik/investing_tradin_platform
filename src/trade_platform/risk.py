...
class KillSwitchRegistry:
    ...
    def blocks(self, instrument_id: str, strategy_version: str, *, account_id: str | None = None, broker_name: str | None = None, exchange: str | None = None, asset_class: str | None = None) -> bool:
        return bool(_kill_switch_candidates(instrument_id, strategy_version, account_id, broker_name, exchange, asset_class) & self._active_scopes)
...
class PreTradeExecutionContext:
    ...
    def validate(self) -> None:
        if self.expected_slippage_fraction < 0 or not Decimal("0") <= self.event_risk <= Decimal("1") or self.available_buying_power < 0:
...
def _validate_kill_switch_scope(scope: str) -> None:
    ...
    if separator != ":" or not value.strip() or prefix not in {"ACCOUNT", "ASSET_CLASS", "BROKER", "EXCHANGE", "INSTRUMENT", "STRATEGY"}:
        raise ValueError("invalid_kill_switch_scope")


def _kill_switch_candidates(instrument_id: str, strategy_version: str, account_id: str | None, broker_name: str | None, exchange: str | None, asset_class: str | None) -> set[str]:
...
class SQLiteKillSwitchRegistry:
    ...
    def __init__(self, database_path: str | Path = ":memory:") -> None:
        ...
        self._connection.execute("CREATE TABLE IF NOT EXISTS kill_switch_events (sequence INTEGER PRIMARY KEY AUTOINCREMENT, event_id TEXT NOT NULL UNIQUE, scope TEXT NOT NULL, active INTEGER NOT NULL, actor TEXT NOT NULL, occurred_at TEXT NOT NULL)")
        self._connection.commit()

    def activate(self, scope: str, *, actor: str = "system") -> KillSwitchEvent:
        return self._append(scope, True, actor)

    def deactivate(self, scope: str, *, explicit_operator_approval: bool, actor: str = "operator") -> KillSwitchEvent:
        if not explicit_operator_approval:
            raise PermissionError("Kill switch reactivation requires explicit operator approval.")
        return self._append(scope, False, actor)

    def blocks(self, instrument_id: str, strategy_version: str, *, account_id: str | None = None, broker_name: str | None = None, exchange: str | None = None, asset_class: str | None = None) -> bool:
        candidates = _kill_switch_candidates(instrument_id, strategy_version, account_id, broker_name, exchange, asset_class)
        return bool(candidates & self.active_scopes())

    def active_scopes(self) -> set[str]:
        rows = self._connection.execute("SELECT scope, active FROM kill_switch_events WHERE sequence IN (SELECT MAX(sequence) FROM kill_switch_events GROUP BY scope)").fetchall()
        return {row[0] for row in rows if bool(row[1])}

    def events(self, scope: str | None = None) -> tuple[KillSwitchEvent, ...]:
        if scope is not None:
            rows = self._connection.execute("SELECT event_id, scope, active, actor, occurred_at FROM kill_switch_events WHERE scope = ? ORDER BY sequence", (scope,)).fetchall()
        else:
            rows = self._connection.execute("SELECT event_id, scope, active, actor, occurred_at FROM kill_switch_events ORDER BY sequence").fetchall()
        return tuple(KillSwitchEvent(UUID(row[0]), row[1], bool(row[2]), row[3], datetime.fromisoformat(row[4])) for row in rows)
    ...
    def _append(self, scope: str, active: bool, actor: str) -> KillSwitchEvent:
        ...
        self._connection.execute("INSERT INTO kill_switch_events (event_id, scope, active, actor, occurred_at) VALUES (?, ?, ?, ?, ?)", (str(event.event_id), scope, int(active), actor, event.occurred_at.isoformat()))
        self._connection.commit()
        return event
...
class SQLiteIdempotencyLedger:
    ...
    def __init__(self, database_path: str | Path = ":memory:") -> None:
        self._connection = sqlite3.connect(str(database_path), check_same_thread=False)
        self._connection.execute("CREATE TABLE IF NOT EXISTS risk_idempotency_claims (intent_id TEXT PRIMARY KEY, claimed_at TEXT NOT NULL)")
        self._connection.commit()

    def claim(self, intent_id: object) -> bool:
        try:
            self._connection.execute("INSERT INTO risk_idempotency_claims VALUES (?, ?)", (str(intent_id), utc_now().isoformat()))
...
class SQLiteDailyNotionalLedger:
    ...
    def __init__(self, database_path: str | Path = ":memory:") -> None:
        ...
        self._connection.execute("CREATE TABLE IF NOT EXISTS risk_daily_notional (intent_id TEXT PRIMARY KEY, account_id TEXT NOT NULL, trading_day TEXT NOT NULL, notional TEXT NOT NULL)")
    ...
    def reserve(self, intent: OrderIntent, *, day: date, maximum: Decimal) -> bool:
        ...
        try:
            existing = self._connection.execute("SELECT 1 FROM risk_daily_notional WHERE intent_id = ?", (str(intent.intent_id),)).fetchone()
            rows = self._connection.execute("SELECT notional FROM risk_daily_notional WHERE account_id = ? AND trading_day = ?", (intent.account_id, day.isoformat())).fetchall()
            total = sum((Decimal(row[0]) for row in rows), Decimal("0"))
            if existing is not None or total + intent.notional > maximum:
                self._connection.execute("ROLLBACK")
                return False
            self._connection.execute("INSERT INTO risk_daily_notional VALUES (?, ?, ?, ?)", (str(intent.intent_id), intent.account_id, day.isoformat(), str(intent.notional)))
...
class SQLiteRiskDecisionStore:
    ...
    def __init__(self, database_path: str | Path = ":memory:") -> None:
        self._connection = sqlite3.connect(str(database_path), check_same_thread=False)
        self._connection.execute("CREATE TABLE IF NOT EXISTS risk_decisions (decision_id TEXT PRIMARY KEY, intent_id TEXT NOT NULL, decision TEXT NOT NULL, reasons_json TEXT NOT NULL, decided_at TEXT NOT NULL)")
        self._connection.commit()

    def append(self, decision: RiskDecision) -> None:
        self._connection.execute("INSERT INTO risk_decisions VALUES (?, ?, ?, ?, ?)", (str(decision.decision_id), str(decision.intent_id), decision.decision.value, json.dumps(decision.reasons), decision.decided_at.isoformat()))
        self._connection.commit()

    def get(self, decision_id: UUID) -> RiskDecision:
        row = self._connection.execute("SELECT * FROM risk_decisions WHERE decision_id = ?", (str(decision_id),)).fetchone()
        if row is None:
            raise KeyError(str(decision_id))
        return RiskDecision(UUID(row[0]), UUID(row[1]), RiskDecisionType(row[2]), tuple(json.loads(row[3])), datetime.fromisoformat(row[4]))
    ...
    def decisions_for_intent(self, intent_id: UUID) -> tuple[RiskDecision, ...]:
        ...
        return tuple(RiskDecision(UUID(row[0]), UUID(row[1]), RiskDecisionType(row[2]), tuple(json.loads(row[3])), datetime.fromisoformat(row[4])) for row in rows)
...
class RiskEngine:
    ...
    def assess(
        ...
        reasons: list[str] = []
        if not self._idempotency_ledger.claim(intent.intent_id):
            decision = RiskDecision.create(intent.intent_id, RiskDecisionType.REJECT, ["duplicate_intent"])
        ...
        if min(market.data_quality_score, signal.data_quality_score) < self._policy.minimum_data_quality:
        ...
        if self._kill_switches.blocks(intent.instrument_id, signal.strategy_version, account_id=intent.account_id):
        ...
        if execution is None:
            ...
            if execution.expected_slippage_fraction > self._policy.maximum_expected_slippage_fraction:
        ...
        if not reasons and not self._daily_notional_ledger.reserve(intent, day=now.date(), maximum=self._policy.maximum_daily_order_notional):
