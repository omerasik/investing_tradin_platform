...
from .broker_adapter import BrokerAccountSnapshot, BrokerOrderEvent, InternalAccountState, PaperBrokerAdapter, reconcile_broker_account
...
def evaluate_pre_trade(intent: OrderIntent, risk_decision: RiskDecision, portfolio_decision: PortfolioRiskDecision) -> PreTradeDecision:
    if risk_decision.intent_id != intent.intent_id:
        raise BrokerSyncError("risk_decision_intent_mismatch")
    reasons = tuple(f"individual_risk:{reason}" for reason in risk_decision.reasons) + tuple(f"portfolio_risk:{reason}" for reason in portfolio_decision.reasons)
    return PreTradeDecision(uuid4(), intent.intent_id, risk_decision.decision is RiskDecisionType.APPROVE and portfolio_decision.approved, reasons, utc_now())
...
class SQLitePreTradeDecisionStore:
    ...
    def __init__(self, database_path: str | Path = ":memory:") -> None:
        ...
        self._connection.execute("CREATE TABLE IF NOT EXISTS pre_trade_decisions (decision_id TEXT PRIMARY KEY, intent_id TEXT NOT NULL, approved INTEGER NOT NULL, reasons_json TEXT NOT NULL, decided_at TEXT NOT NULL)")
        self._connection.commit()

    def append(self, decision: PreTradeDecision) -> None:
        self._connection.execute("INSERT INTO pre_trade_decisions VALUES (?, ?, ?, ?, ?)", (str(decision.decision_id), str(decision.intent_id), int(decision.approved), json.dumps(decision.reasons), decision.decided_at.isoformat()))
        self._connection.commit()

    def get(self, decision_id: UUID) -> PreTradeDecision:
        row = self._connection.execute("SELECT * FROM pre_trade_decisions WHERE decision_id = ?", (str(decision_id),)).fetchone()
        if row is None:
            raise KeyError(str(decision_id))
        return PreTradeDecision(UUID(row[0]), UUID(row[1]), bool(row[2]), tuple(json.loads(row[3])), datetime.fromisoformat(row[4]))

    def latest_for_intent(self, intent_id: UUID) -> PreTradeDecision:
        row = self._connection.execute("SELECT * FROM pre_trade_decisions WHERE intent_id = ? ORDER BY decided_at DESC, rowid DESC LIMIT 1", (str(intent_id),)).fetchone()
        if row is None:
...
class SQLiteBrokerEventStore:
    ...
    def __init__(self, database_path: str | Path = ":memory:") -> None:
        self._connection = sqlite3.connect(str(database_path), check_same_thread=False)
        self._connection.execute("CREATE TABLE IF NOT EXISTS broker_events (event_id TEXT PRIMARY KEY, broker_name TEXT NOT NULL, sequence INTEGER NOT NULL, external_event_id TEXT NOT NULL UNIQUE, intent_id TEXT NOT NULL, event_type TEXT NOT NULL, status TEXT NOT NULL, occurred_at TEXT NOT NULL)")
        self._connection.execute("CREATE TABLE IF NOT EXISTS broker_cursors (broker_name TEXT PRIMARY KEY, sequence INTEGER NOT NULL)")
        self._connection.commit()

    def last_sequence(self, broker_name: str) -> int:
        row = self._connection.execute("SELECT sequence FROM broker_cursors WHERE broker_name = ?", (broker_name,)).fetchone()
    ...
    def append(self, broker_name: str, event: BrokerOrderEvent) -> bool:
        ...
        try:
            self._connection.execute("INSERT INTO broker_events VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (str(event.event_id), broker_name, event.sequence, event.external_event_id, str(event.client_intent_id), event.event_type, event.status.value, event.occurred_at.isoformat()))
        except sqlite3.IntegrityError:
            return False
        previous = self.last_sequence(broker_name)
        if event.sequence > previous:
            self._connection.execute("INSERT INTO broker_cursors VALUES (?, ?) ON CONFLICT(broker_name) DO UPDATE SET sequence = excluded.sequence", (broker_name, event.sequence))
        self._connection.commit()
        return True
...
class PaperBrokerSyncService:
    ...
    def __init__(self, oms: SQLitePaperOms, adapter: PaperBrokerAdapter, event_store: SQLiteBrokerEventStore, alerts: SQLiteOperationalAlertStore | None = None, pre_trade_store: SQLitePreTradeDecisionStore | None = None, assessment_store: "SQLitePreTradeAssessmentStore | None" = None, policy_registry: "SQLitePolicyRegistry | None" = None) -> None:
        if adapter.capabilities.network_connected:
            raise BrokerSyncError("network_connected_adapter_not_permitted")
        self._oms, self._adapter, self._events, self._alerts, self._pre_trade_store = oms, adapter, event_store, alerts, pre_trade_store
        self._assessment_store, self._policy_registry = assessment_store, policy_registry

    def _submit_checked_assessment(self, intent: OrderIntent, risk_decision: RiskDecision, portfolio_decision: PortfolioRiskDecision) -> PaperOrder:
        ...
        if not decision.approved:
            return self._oms.transition(pending.intent.intent_id, OrderStatus.RISK_REJECTED, actor="pre_trade")
        approved = self._oms.transition(pending.intent.intent_id, OrderStatus.APPROVED, actor="risk")
        try:
            acknowledged = self._adapter.submit(approved)
        except Exception as error:
            self._oms.transition(intent.intent_id, OrderStatus.SUBMISSION_PENDING, actor="broker_sync")
        ...
        for status in (OrderStatus.SUBMISSION_PENDING, OrderStatus.SUBMITTED, OrderStatus.ACKNOWLEDGED):
    ...
    def submit_after_assessment(self, intent: OrderIntent, assessment: PreTradeAssessment) -> PaperOrder:
        ...
        try:
            ...
            portfolio_document = self._policy_registry.get("portfolio", stored.portfolio_policy_version)
        except ValueError as error:
            raise BrokerSyncError("policy_evidence_unavailable") from error
        if stored.risk_policy_digest != risk_document.digest or stored.portfolio_policy_digest != portfolio_document.digest:
        ...
        if portfolio_decision is None:
            portfolio_decision = PortfolioRiskDecision(False, ("assessment_incomplete",), Decimal("0"), Decimal("0"), Decimal("0"), None, None, None)
    ...
    def sync_events(self) -> tuple[StoredBrokerEvent, ...]:
        ...
        for event in self._adapter.order_events(after_sequence=self._events.last_sequence(broker_name)):
            if not self._events.append(broker_name, event):
                continue
            if event.event_type == "FILL":
                try:
                    self._oms.ingest_fill(event.details["external_fill_id"], event.client_intent_id, Decimal(event.details["quantity"]), Decimal(event.details["price"]), occurred_at=event.occurred_at)
                ...
                try:
                    self._oms.amend(event.client_intent_id, quantity=Decimal(event.details["quantity"]), limit_price=Decimal(event.details["limit_price"]), actor="broker_sync")
                except (KeyError, ValueError, PaperOmsError) as error:
                    raise BrokerSyncError("broker_replace_event_rejected") from error
            stored.append(StoredBrokerEvent(event.event_id, broker_name, event.sequence, event.external_event_id, event.client_intent_id, event.event_type, event.status, event.occurred_at))
        return tuple(stored)
    ...
    def reconcile(self, internal: InternalAccountState) -> ReconciliationResult:
        ...
        ingested_at = utc_now()
        if ingested_at < snapshot.as_of:
            ingested_at = snapshot.as_of
        self._oms.record_reconciliation_with_account(snapshot, recorded, healthy=self._adapter.health(), ingested_at=ingested_at)
        if not recorded.complete and self._alerts is not None:
            self._alerts.raise_alert(source="broker_sync", code="RECONCILIATION_FAILED", severity=AlertSeverity.CRITICAL, resource=f"account:{snapshot.account_id}", details={"discrepancies": ",".join(recorded.discrepancies)})
        return recorded
