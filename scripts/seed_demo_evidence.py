...
DEMO_AT = datetime.now(UTC).replace(microsecond=0)
...
def _seed_research_chain(database: PostgresDatabase, core: dict[str, UUID]) -> dict[str, UUID]:
    ids = {name: stable_id(name) for name in (
        ...
        "strategy-2", "strategy-version-2", "experiment-2", "scorecard-2",
    )}
    ...
    with database.transaction() as connection, connection.cursor() as cursor:
        ...
        # A second, independent research scenario (different family/status/instrument) so
        # discovery filters and pagination in the research workspaces have more than one row.
        _insert(cursor, "INSERT INTO strategy_definitions VALUES (%s,'MEAN_REVERSION','Mean reversion synthetic research hypothesis; no live authority.',%s) ON CONFLICT (strategy_id) DO NOTHING", (ids["strategy-2"], DEMO_AT))
        _insert(cursor, "INSERT INTO strategy_versions VALUES (%s,%s,'mean-reversion-v1',%s::jsonb,'demo-cost-model-v1','demo-capacity-v1',%s::jsonb,%s) ON CONFLICT (strategy_version_id) DO NOTHING", (ids["strategy-version-2"], ids["strategy-2"], json.dumps([f"{name}:1.0.0" for name in ("demo_return", "demo_trend", "demo_momentum", "demo_volatility")]), json.dumps(strategy_contract), DEMO_AT))
        _insert(cursor, "INSERT INTO research_experiments VALUES (%s,%s,%s,%s::jsonb,%s::jsonb,%s,%s) ON CONFLICT (experiment_id) DO NOTHING", (ids["experiment-2"], ids["strategy-version-2"], ids["dataset-version"], json.dumps({"scenario": f"{DEMO_SEED_VERSION}-2"}), json.dumps({"total_return": "-0.01", "classification": DEMO_SOURCE}), digest({"experiment-2": DEMO_SEED_VERSION}), DEMO_AT))
...
def _seed_investment_paper_news_sre(database: PostgresDatabase, core: dict[str, UUID], ids: dict[str, UUID]) -> None:
    ...
    with database.transaction() as connection, connection.cursor() as cursor:
        ...
        # A second, unvalidated candidate signal on a different instrument/strategy for filter coverage.
        second_signal_payload = {"direction": "SELL", "strength": "0.4", "confidence": "0.3", "data_quality_score": "0.8", "explanation": "Synthetic candidate signal awaiting validation.", "contradicting_evidence": []}
        _insert(cursor, "INSERT INTO runtime_signal_proposals VALUES (%s,'DEMO:XNAS:DEMO_EQ_B','mean-reversion-v1',%s,%s,%s::jsonb) ON CONFLICT (signal_id) DO NOTHING", (ids["blocked-signal"], DEMO_AT - timedelta(hours=1), DEMO_AT + timedelta(days=2), json.dumps(second_signal_payload)))
