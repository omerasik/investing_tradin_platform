...
DEMO_AT = datetime(2025, 2, 3, 15, tzinfo=UTC)
...
def _seed_research_chain(database: PostgresDatabase, core: dict[str, UUID]) -> dict[str, UUID]:
    ...
    with database.transaction() as connection, connection.cursor() as cursor:
        ...
        _insert(cursor, "INSERT INTO validation_packages(package_id,strategy_version_id,dataset_version_id,cost_model_version,content_hash,status,created_at,limitations,feature_versions) VALUES (%s,%s,%s,'demo-cost-model-v1',%s,'REVIEW_REQUIRED_OR_BLOCKED',%s,%s::jsonb,%s::jsonb) ON CONFLICT (package_id) DO NOTHING", (ids["validation-package"], ids["strategy-version"], ids["dataset-version"], digest({"validation": DEMO_SEED_VERSION}), DEMO_AT, json.dumps(["Synthetic engineering evidence only."]), json.dumps(["demo_return:1.0.0", "demo_trend:1.0.0", "demo_momentum:1.0.0", "demo_volatility:1.0.0"])))
