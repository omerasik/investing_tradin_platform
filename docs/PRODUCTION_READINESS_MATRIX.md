...
| Audit events | `SQLiteAuditStore` (audit.py:22) dev/paper; `PostgresAuditStore` (postgres_audit.py:29) prod | api.py audit endpoints | Any service via `.append()` | Append-only, no update/delete exposed; Postgres variant additionally immutable at the schema level (`production_audit_events` table) | `SQLiteAuditStore` remains explicitly dev/paper-only per its own docstring; `PostgresAuditStore` is prod-capable and is composed unconditionally into `production` runtime by `trade_platform.runtime_app` (Module 3D) | ACCEPTABLE | Resolved by Module 3D — see `docs/MODULE_3D_PRODUCTION_IDENTITY_SECRETS_AUDIT.md` |
