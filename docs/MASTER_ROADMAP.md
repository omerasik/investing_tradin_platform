...
| RQ-024 | Platform §23: security | PARTIAL | `security.py`, `external_identity.py`, `config.py`, `audit.py`, migration `0029`, API/dashboard header configuration, `.github/workflows/verify.yml`; API/config/identity/role/browser tests | Cycle 222 run `32547343608` verifies a provider-independent external verified-session boundary, exact issuer/audience/time/MFA/group mapping, durable allow/deny evidence and fail-closed audit availability. The local default remains one static token, and no IdP/JWT verifier is activated. Managed OIDC/RBAC/session lifecycle/CSRF, strict nonce-based dashboard CSP, secret manager/KMS, SSRF hardening, encrypted off-site backups, penetration evidence and incident operations remain incomplete. Never place secrets in frontend or source. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `Dockerfile`, `.github/workflows/verify.yml`, dashboard smoke and Playwright/Axe | Cycle 222 PR-head run `32547343608` passes all 408 tests without skips, all 113 restore checks, the external-session/audit security suite, signed archive provenance/SBOM verification and every existing gate. The full report retains 26 vendor-unfixed HIGH/CRITICAL findings and zero fixable ones. Load/performance, manual assistive-tech, registry-native OCI signing and production deployment testing remain incomplete. |
| RQ-027 | Platform §§29–35: phased operations, reports, cost governance, runbooks, live promotion | PARTIAL | `docs/*.md`, `operational_alerts.py`, `backup_recovery.py`, `strategy_promotion.py` | The roadmap and runbooks are traceable through Cycle 222 and distinguish verified provider-neutral evidence from external activation blockers; promotion is fail-closed to `REVIEW_REQUIRED`, not live. Scheduled operational reports/cost service, external provider runbooks executed under authorization, and production operations remain incomplete. Live pilot remains `REJECTED_WITH_REASON`: live execution is prohibited pending separate explicit authorization. |
...
| RQ-024 | PARTIAL | Development bearer auth, explicit static roles, provider-independent verified-session mapping, immutable PostgreSQL authorization decisions, signed evidence and HTTP/security gates. | Cycle 222 run `32547343608` proves issuer/audience/time/MFA/group failure cases, mandatory durable audit composition, policy/decision restart/immutability and no raw token/session persistence. | No selected/activated OIDC verifier/IdP, managed RBAC/session lifecycle/CSRF, nonce-based dashboard CSP, managed secrets/KMS or penetration evidence. | RQ-002, RQ-022–RQ-027. | Identity/cloud vendor decisions for final integration. | Select and independently accept an IdP/verifier, then exercise key rotation, revocation and identity lifecycle without weakening viewer default. | Threat model and production identity/secrets controls pass independent security acceptance. |
...
2. **RQ-024/RQ-026 security and deployment quality:** compose the verified
   external-session contract with a selected IdP only after approval; add
   nonce-based CSP and measured performance budgets; retain manual
...
- The Next dashboard applies corresponding headers to `/` and `/api/*` while
  retaining its separate view-token boundary. Framework hydration still needs
  inline script/style CSP allowances, which remains an explicit limitation.
