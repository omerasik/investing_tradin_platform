...
| RQ-022 | Platform §20: operator dashboard and all listed workflows | PARTIAL | `operator_dashboard.py`, protected routes in `api.py`, `web/app/page.tsx`, `web/app/operator-contracts.ts`, `web/app/api/authorities/route.ts`, `web/proxy.ts`, `web/e2e/cycle208.spec.ts`; API/PostgreSQL tests | Cycle 208 adds protected typed read workspaces for Feature Authority, Scorecard V2, Regime V2, Portfolio Construction V2, News/Event V2 and SRE V2 plus a non-authoritative Command Center summary. The Next proxy is GET-only/allowlisted, backend credentials stay server-side, configured page viewing has a separate bearer boundary, and no workspace adds execution, risk-increase or provider activation controls. Latest exact-mainline run `32278879487` passed the no-skip PostgreSQL query-plan coverage and all ten configured browser scenarios. Signal/Risk/Model/Execution workspaces, charts, production RBAC/MFA and complete accessibility automation remain incomplete. |
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0022`, research/risk/operations authorities; PostgreSQL integration/restore tests | Run `32280796788` applies migration 0022 and hash-matches all 97 restored critical tables, including six immutable social/narrative authorities. Legacy research SQLite, object storage/queues, retention and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, dashboard smoke and Playwright | Latest exact-mainline run `32278879487` applied migration 0021, ran all 351 tests without skips, matched all 91 restored tables and passed the complete workflow, including the 120/120 mypy ratchet, zero-error 28-file critical slice, dashboard smoke and ten configured PostgreSQL browser scenarios. Performance/load, complete accessibility, container and deployment testing remain incomplete. |
...
| RQ-018 | PARTIAL | Rich signal proposals, nine-stage validation, append-only lifecycle, expiry check and risk-contract adapter. | Signal/coordinator tests verify skipped/terminal/stale/unvalidated proposals fail closed. | No expiry worker, complete authoritative integrations, reason workspace or full managed PostgreSQL lifecycle. | RQ-004, RQ-009–RQ-011, RQ-016, RQ-019–RQ-020, RQ-022–RQ-023. | Empirical signals require authorized inputs; core workflow is unblocked. | Add expiry scheduling, reason API and protected Signal Explorer. | Every signal is PIT-bound, auto-expires, is explainable in UI and cannot bypass risk. |
...
| RQ-022 | PARTIAL | Protected typed Command Center and six authority workspaces via GET-only allowlisted proxy. | Exact-mainline run `32278879487` passes query-plan coverage, build, smoke and ten configured browser scenarios. | Missing Signal/Risk/Model/Execution workspaces, interactive charts, full accessibility and production auth. | RQ-017–RQ-020, RQ-024, RQ-026. | Production identity design affects final access controls only. | Build Signal Explorer/reason views and automated accessibility coverage. | All required workflows are protected, read-authoritative, accessible and browser-verified. |
| RQ-023 | PARTIAL | 22 Alembic migrations and 97-table restore allow-list; SQLite retained for local research. | Run `32280796788` applies migration 0022 and matches all 97 restored tables with recovery protections. | No queue/object-store/job/retention authority or deployed backup topology; some research stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners for production. | Define durable job/retention/object evidence and classify remaining SQLite surfaces. | Production authorities, lifecycle/retention, backups and restore are migration-managed and tested. |
...
| RQ-026 | PARTIAL | 357-test suite, migration/restore, Ruff/mypy/Bandit/audit/SBOM/secrets, locked frontend build, smoke and Playwright. | PR run `32280796788` passes all 357 tests without skips, all 97 restored tables and every configured quality/security/frontend/browser gate. | Load/performance, comprehensive accessibility, container/deployment and broader chaos tests are absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity for production-grade load tests. | Add accessibility, performance-budget and container validation gates. | Required functional, security, performance, recovery and deployment suites pass without waived critical findings. |
...
1. **RQ-018/RQ-022 signal operations:** expiry scheduling, reason APIs, Signal
   Explorer and accessibility evidence.
2. **RQ-019/RQ-020 risk and paper operations:** per-trade loss/stop-gap,
   execution-quality and shadow evidence without a network broker.
3. **RQ-023/RQ-025 durable operations:** job/retention/object evidence,
   scheduler monitoring and alert-routing boundaries.
4. **RQ-024/RQ-026 security and deployment quality:** role/session abstractions,
   headers, accessibility, performance budgets and non-root container checks.
5. **RQ-016/RQ-017 model/agent evaluation:** holdout/calibration/explainability
   and retrieval evidence, still non-executing and fixture-labelled.
6. **RQ-028–RQ-030 upstream adoption:** refresh identity inventory, complete
   isolated scans, then run one restricted fixture-only POC.
7. **External activation lane:** keep RQ-003–RQ-008 provider decisions concise;
