...
`main` at `2fd0a38`. The latest exact-mainline evidence is GitHub Actions run
`32277549784`: all 351 Python tests ran without skips, migration head
...
| RQ-008 | Platform §5.6: lawful social/narrative intelligence | NOT_STARTED | — | No lawful-source authority, aggregation, spam/bot/pump detection, narrative clustering, crowding or separate sentiment categories. Provider-neutral core engineering is unblocked and selected for Cycle 210; real-source activation still requires explicit terms and credentials. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, dashboard smoke and Playwright | Latest exact-mainline run `32277549784` applied migration 0021, ran all 351 tests without skips, matched all 91 restored tables and passed the complete workflow, including the 120/120 mypy ratchet, zero-error 28-file critical slice, dashboard smoke and ten configured PostgreSQL browser scenarios. Performance/load, complete accessibility, container and deployment testing remain incomplete. |
...
|---|---|---|---|---|---|---|---|---|
| RQ-001 | PARTIAL | Paper-only configuration, separated trading/investment records, deterministic risk, PostgreSQL configured paper runtime and immutable evidence. | Fifteen-invariant P0 audit plus run `32277549784`; configured runtime approval/rejection, fill, reconciliation and restart tests. | No production identity/approval system or complete multi-account capital-allocation policy. | RQ-019, RQ-021, RQ-024. | Capital allocation and any future live authorization require named owner approval. | Add immutable multi-account capital-policy and approval evidence without execution authority. | Approved policies, separation and fail-closed enforcement are tested end to end; live remains separately prohibited. |
| RQ-002 | PARTIAL | Modular Python/FastAPI/Next monolith, PostgreSQL/Alembic, append-only authorities and CI. | Migration 0021, 91-table restore, build and browser gates passed in run `32277549784`. | No queue/cache/object-store authority, container image, IaC or deployed environment. | RQ-023–RQ-027. | Cloud account, region, budget and ownership are needed only for deployment. | Define a non-root reproducible container and disposable migration/health validation. | Reproducible local/staging deployment passes migration, health, rollback and security gates. |
...
| RQ-005 | PARTIAL | PIT PostgreSQL filings/facts with revisions, standardization and formula provenance. | Cycle 14 restart/restore/PIT tests remain green in run `32277549784`. | No real SEC retrieval, estimates, guidance, insider or ownership feed. | RQ-003, RQ-004, RQ-009, RQ-021. | SEC identifying configuration and/or licensed-source terms. | Implement a controlled SEC-source adapter and fair-access activation package. | Authorized filings reconcile to source documents and drive PIT features/investment evidence. |
...
| RQ-008 | NOT_STARTED | No social/narrative domain authority or connector. | Repository inventory confirms no implementation or tests. | No lawful-source policy, aggregate windows, spam/bot/pump controls, narrative clusters, crowding or price divergence. | RQ-003, RQ-004, RQ-007, RQ-009. | Only real-source activation needs approved terms/credentials; core engineering is unblocked. | Cycle 210: build a provider-neutral, rights-gated social/narrative evidence core with deterministic tests. | Immutable PIT aggregates distinguish sentiment categories, detect manipulation/crowding, preserve provenance and fail closed on rights/quality. |
...
| RQ-022 | PARTIAL | Protected typed Command Center and six authority workspaces via GET-only allowlisted proxy. | Cycle 208 run `32277549784` passes query-plan coverage, build, smoke and ten configured browser scenarios. | Missing Signal/Risk/Model/Execution workspaces, interactive charts, full accessibility and production auth. | RQ-017–RQ-020, RQ-024, RQ-026. | Production identity design affects final access controls only. | Build Signal Explorer/reason views and automated accessibility coverage. | All required workflows are protected, read-authoritative, accessible and browser-verified. |
| RQ-023 | PARTIAL | 21 Alembic migrations, 91 restored tables, normalized PG authorities; SQLite retained for local research. | Exact-mainline restore/reconciliation and critical DB protections pass in run `32277549784`. | No queue/object-store/job/retention authority or deployed backup topology; some research stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners for production. | Define durable job/retention/object evidence and classify remaining SQLite surfaces. | Production authorities, lifecycle/retention, backups and restore are migration-managed and tested. |
...
1. **Cycle 210 — RQ-008 social/narrative core:** close the only unstarted
   provider-neutral platform domain with immutable rights/provenance, PIT
   aggregation, manipulation controls and deterministic fixture evidence.
