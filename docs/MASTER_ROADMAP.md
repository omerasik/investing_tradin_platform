...
| RQ-022 | Platform §20: operator dashboard and all listed workflows | PARTIAL | `operator_dashboard.py`, protected routes in `api.py`, `web/app/page.tsx`, `web/app/operator-contracts.ts`, GET-only authority proxy and Playwright; API/PostgreSQL tests | Cycle 211 adds a protected PIT Signal Explorer with expiry, validation, reason, contradicting-evidence and lifecycle views and no mutation/execution controls. PR-head run `32283931250` verifies the expanded eleven-scenario protected PostgreSQL browser matrix. Risk/Model/Execution workspaces, charts, production RBAC/MFA and comprehensive accessibility automation remain incomplete. |
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0023`, research/risk/operations authorities; PostgreSQL integration/restore tests | Cycle 211 adds an immutable lifecycle table and explicitly includes proposal/validation/lifecycle authorities in the 100-table restore manifest; PR-head run `32283931250` verifies migration 0023 and all 100 restored tables. Legacy research SQLite, object storage/queues, retention and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, dashboard smoke and Playwright | Cycle 211 PR-head run `32283931250` ran all 358 tests without skips, matched all 100 restore-critical tables and passed the quality, security, supply-chain, production-build and eleven-scenario protected browser gates. Performance/load, comprehensive accessibility, container and deployment testing remain incomplete. |
