...
| RQ-019 | Platform §16: independent deterministic risk engine, limits, stress, kill switches | PARTIAL | `risk.py`, `execution_evidence.py`, `quotes.py`, `portfolio_evidence.py`, `pretrade_context.py`, `pretrade_assessment.py`, `portfolio_context.py`, `portfolio_risk.py`, `broker_sync.py`, `api.py`, migration `0024`; risk/pre-trade/policy/runtime/PostgreSQL tests | Cycle 212 PR-head run `32286392247` verifies policy-bound per-trade stop/distance/raw-loss/gap-loss controls and immutable PostgreSQL calculations. The engine binds the validated signal direction, entry range and stop to reviewed limits, and composed paper runtimes reject incomplete policies. No calibrated empirical gap/factor/correlation models, live calendar/halt/quote/broker sources, fully atomic snapshot/reconciliation transaction, cancellation behavior, broad notification producers or generalized failure drills exist. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0024`, research/risk/operations authorities; PostgreSQL integration/restore tests | Cycle 212 PR-head run `32286392247` verifies migration 0024's per-trade payload-shape constraint and all 100 restored tables. Legacy research SQLite, object storage/queues, retention and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, dashboard smoke and Playwright | Cycle 212 PR-head run `32286392247` passes all 364 tests without skips, all 100 restore checks, the 117/117 mypy ratchet, zero-error 34-file slice and every configured workflow gate. Performance/load, comprehensive accessibility, container and deployment testing remain incomplete. |
...
| RQ-019 | PARTIAL | Deterministic individual/portfolio risk, scoped kill switches, PG reservations/decisions, authoritative context and policy-bound stop/gap loss. | PR-head run `32286392247` verifies missing/wrong-side stops, entry/direction mismatch, distance/raw/gap loss rejection, keyed restart and runtime policy completeness in PostgreSQL. | Gap buffer is policy-prescribed rather than empirically calibrated; fully atomic snapshot/reconciliation and cancellation/failure operations remain incomplete. | RQ-003–RQ-004, RQ-011, RQ-018, RQ-020, RQ-023. | Calibration/live sources only; deterministic extensions are unblocked. | Add durable risk-violation notifications and failure/concurrency drills. | Complete limits/stress/kill switches are atomic, explainable and failure-tested under concurrency. |
