...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0026`, research/risk/operations authorities; PostgreSQL integration/restore tests | Cycle 214 PR-head run `32290318303` verifies migration 0026, immutable paper operations evidence and all 104 restored tables. Legacy research SQLite, object storage/queues, retention and production deployment remain incomplete. |
...
| RQ-025 | Platform §§24–25: observability, backup/recovery and failure injection | PARTIAL | `observability.py`, `observability_sre_v2.py`, `shadow_mode.py`, `execution_quality.py`, `operational_alerts.py`, recovery modules, migrations `0021`, `0025`–`0026`; related tests and restore drill | Cycles 207/213 verify SRE and atomic risk-alert evidence; Cycle 214 adds atomic local execution-quality/shadow-rehearsal alerts and injected alert-write rollback in run `32290318303`. External routing, approved production SLO attainment, managed telemetry, scheduler monitoring, encrypted off-site backups, provider failover, IaC/deploy/rollback and staged soak remain incomplete. |
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, dashboard smoke and Playwright | Cycle 214 PR-head run `32290318303` passes all 372 tests without skips, all 104 restore checks, the 117/117 mypy ratchet, zero-error 36-file slice and every configured workflow gate. Performance/load, comprehensive accessibility, container and deployment testing remain incomplete. |
...
| RQ-019 | PARTIAL | Deterministic individual/portfolio risk, scoped kill switches, PG reservations/decisions, policy-bound stop/gap loss and atomic rejection/quality alerts. | Runs `32288130114` and `32290318303` verify reservation safety, concurrent replay, restart and injected alert rollback. | Gap buffer is policy-prescribed rather than empirically calibrated; snapshot/reconciliation reads, cancellation behavior, broad producers and external delivery remain incomplete. | RQ-003–RQ-004, RQ-011, RQ-018, RQ-020, RQ-023. | Calibration/live sources only; deterministic extensions are unblocked. | Add durable scheduler monitoring and alert-routing boundaries. | Complete limits/stress/kill switches are atomic, explainable and failure-tested under concurrency. |
...
| RQ-023 | PARTIAL | 26 Alembic migrations and 104-table restore allow-list; SQLite retained for local research. | PR-head run `32290318303` proves migration 0026 and all 104 restore-critical tables. | No queue/object-store/job/retention authority or deployed backup topology; some research stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners for production. | Define durable job/retention/object evidence and classify remaining SQLite surfaces. | Production authorities, lifecycle/retention, backups and restore are migration-managed and tested. |
...
| RQ-025 | PARTIAL | Immutable SRE evidence plus PostgreSQL risk/quality/divergence alert transitions and transactional failure rollback. | Cycles 207, 213 and 214 CI verify deterministic SRE plus atomic rejection and simulated paper-operations alert evidence. | No managed telemetry, approved SLO, external paging/on-call, scheduler monitoring, off-site backup or soak. | RQ-002, RQ-023–RQ-027. | Cloud/on-call ownership, channels and budget. | Add scheduler monitoring and provider-neutral alert-routing interfaces. | Staging soak, SLOs, paging, backup/restore, rollback and failure drills meet approved targets. |
| RQ-026 | PARTIAL | 372-test suite, migration/104-table restore target, Ruff/mypy/Bandit/audit/SBOM/secrets, locked frontend build, smoke and Playwright. | PR-head run `32290318303` passes all 372 tests, 104 restore checks, 117/117 mypy ratchet, zero-error 36-file slice and every configured gate. | Load/performance, comprehensive accessibility, container/deployment and broader chaos tests are absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity for production-grade load tests. | Add accessibility, performance-budget and container validation gates. | Required functional, security, performance, recovery and deployment suites pass without waived critical findings. |
...
1. **RQ-023/RQ-025 durable operations:** job/retention/object evidence,
   scheduler monitoring and alert-routing boundaries.
