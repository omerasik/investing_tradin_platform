...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0014`, `postgres_repositories.py`, `historical_market_data.py`, `data_health.py`, `pit_fundamentals.py`, `pit_macro.py`, `feature_authority.py`; PostgreSQL integration/restore tests | The Alembic-managed PostgreSQL authority covers normalized instruments/calendars, risk/OMS/reconciliation, immutable validation evidence, provider/source/raw-observation/dataset records, Data Health, PIT fundamentals/macro and versioned feature materializations; database-side immutability and restore evidence are CI-verified. Legacy SQLite application paths, complete research-table migration, object storage/queue architecture, retention operations and production deployment remain incomplete. |
