...
|---|---|---|---|---|
| RQ-001 | Platform §§1–3: separated active trading and investment systems; capital preservation, auditability, paper-only live gate | PARTIAL | `domain.py`, `config.py`, `risk.py`, `investments.py`; `test_config.py`, `test_risk.py`, `test_investments.py` | 74 unit tests passed locally on 2026-07-30. Domains and live-disable guard exist; separate accounts, budgets, policies, approvals and full decision lineage do not. Implement persistent domain schemas and policy-versioning. |
...
| RQ-003 | Platform §5.1: complete instrument master, calendars, identifiers, delistings, actions, mappings | PARTIAL | `domain.py`, `instruments.py`; `test_instruments.py` | Venue-qualified IDs and tick/lot data are tested. Persistent namespace-qualified global/broker/provider identifier mappings, time-bounded venue/symbol history, delisting timestamps, and weekday/holiday/timezone session lookup survive reopen; overlapping symbol mappings are rejected. No base currency/contract size, exchange catalog, half-days/overnight/24x7 calendars, derivatives/continuous futures, crypto/FX conventions, provider ingestion or corporate-action linkage. |
| RQ-004 | Platform §§5.2, 6–8: multi-asset historical/streaming market data, provider architecture, quality, provenance | PARTIAL | `market_data.py`, `data_providers.py`, `quotes.py`, `corporate_actions.py`; `test_market_data.py`, `test_data_providers.py`, `test_quotes.py`, `test_corporate_actions.py` | OHLCV fixture contract, point-in-time query, capability metadata, deterministic retry/rate-limit/pagination/cache/fallback/health contracts, terms-gated public daily CSV adapter, append-only corporate-action records, and an append-only provider-attributed bid/ask/last quote store are tested. No terms approval or live request has been made; no streaming, quote/trade/book/funding/OI provider, persistent cache/health service, corporate-action provider adapter, or multi-provider production activation exists. |
| RQ-005 | Platform §5.3: point-in-time fundamental service | PARTIAL | `fundamentals.py`, `investments.py`, `api.py`; `test_fundamentals.py`, `test_investments.py`, `test_api.py` | Persistent facts preserve statement type, reporting period, filing/effective/ingestion timestamps, provider/source/revision, as-reported value and versioned standardized value. Tests prove historical queries exclude unavailable filings/revisions and retain append-only history. Point-in-time-visible filing facts materialize into investment evidence with deterministic provider/source/revision provenance; authenticated commands retain actor/idempotency evidence and safe replays return original facts. Failed source reads retain a durable command outcome while the API returns a generic 502. No SEC/provider adapter, statement parser, estimates/guidance/insider/ownership catalogue, or feature integration exists. |
| RQ-006 | Platform §5.4: versioned macro service with release/revision timing | PARTIAL | `macro_data.py`; `test_macro_data.py` | Persistent macro observations retain observation period, release, ingestion, actual/expected/prior/dispersion, source and revision. Tests prove historical queries cannot see unreleased, future-revised or delayed-ingestion values. No external FRED/ECB adapter, complete macro catalogue, provider health, or macro-feature integration exists. |
...
| RQ-009 | Platform §5.7: versioned feature registry and feature families | PARTIAL | `feature_platform.py`, `research.py`; `test_feature_platform.py`, `test_research.py` | Versioned definitions now retain owner, source, frequency, timing, lookback, missing/outlier policy, leakage test and range expectations. Point-in-time return, moving average, realized volatility, fundamental and macro-surprise computations persist provenance/versioned values and reject future data. Most liquidity, breadth, derivatives, sentiment and cross-asset families remain absent. |
...
| RQ-012 | Platform §11: vectorized research and realistic validation | PARTIAL | `research.py`, `research_validation.py`, `quant_validation.py`, `strategy_promotion.py`; `test_research_validation.py`, `test_quant_validation.py`, `test_strategy_promotion.py` | Content-addressed SQLite evidence now retains versioned OHLCV-estimated multi-capital capacity, six cost scenarios, frequency-aware latency, seeded bootstrap and trade-order Monte Carlo, deterministic stress fixtures, parameter-neighborhood stability, and BH/PSR/deflated-Sharpe/PBO-style multiple-testing evidence. A promotion decision resolves a persisted complete package instead of trusting supplied IDs, fails closed for missing/version-mismatched/failed evidence, and can only become `REVIEW_REQUIRED`. The test fixtures are deterministic and synthetic; no external market/order-book, full historical stress, or provider-backed execution validation exists. |
| RQ-013 | Platform §§11.1–11.2: high-fidelity event simulation | PARTIAL | `event_backtest.py`, `cross_engine.py`, `paper_execution.py`, `corporate_actions.py`; `test_event_backtest.py`, `test_cross_engine.py`, `test_paper_execution.py`, `test_corporate_actions.py` | Deterministic event engine is exercised for latency, sessions, bid/ask, market/limit/stop/stop-limit, partial fills, fee, square-root impact, cash/position accounting and effective one-time corporate-action application. Durable golden artifacts now retain both exact close-auction parity and declared spread/commission/latency/participation/impact scenarios, with partial/unfilled order observations and raw/explained/unexplained divergence separated. They run immutable signals through event fills plus point-in-time dividend, split and symbol-change accounting. Funding/borrow/margin remain separate helpers; no futures/FX/tax/queue model, delisting golden, or empirical multi-price evidence exists. |
| RQ-014 | Upstream §36.10: independent cross-engine validation | PARTIAL | `cross_engine.py`, `research.py`, `strategy_promotion.py`; `test_cross_engine.py`, `test_research.py`, `test_strategy_promotion.py` | Final-equity/fill/position and fill identity/timestamp/quantity/price/fee mismatch tests pass. Every newly launched vectorized baseline experiment compares final equity and turnover against a separate iterative close-to-close accounting implementation. Durable golden runs exercise the same immutable adjusted-close dataset/signal sequence through event fills and point-in-time dividend/split/symbol-change actions under both parity and pessimistic spread/fee/latency/participation/impact assumptions. Raw divergences are retained separately from declared explanations; the promotion gate blocks absent, strategy-mismatched, or unexplained golden evidence. No three-engine run, event artifact API, delisting golden, or empirical data-backed comparison exists. |
| RQ-015 | Platform §12: strategy quality scorecard | PARTIAL | `strategy_scorecard.py`; `test_strategy_scorecard.py` | Held-out return, volatility, Sharpe/Sortino, drawdown, tail, win/loss, streak and turnover metrics are calculated. Components and explicit complexity/sample/turnover penalties are exposed separately; missing cost/data-quality evidence is not scored as positive. No persistent report, capacity/regime/correlation/live-consistency/signal-decay calculation or real-data evidence. |
...
| RQ-021 | Platform §19: investment intelligence, valuation, themes, thesis drift | PARTIAL | `investments.py`, `investment_providers.py`, `operational_alerts.py`, `api.py`, `web/app/page.tsx`, `web/app/api/alerts/route.ts`, `docs/INVESTMENT_EVIDENCE_PROVIDER_RUNBOOK.md`; `test_investments.py`, `test_investment_providers.py`, `test_operational_alerts.py`, `test_api.py`, `scripts/serve_dashboard_fixture.py` | Append-only theses, source-backed point-in-time facts, transparent DCF records, durable recommendations/reviews, review schedules and deterministic drift assessments are tested across SQLite. The normalized evidence layer includes fixture and configured HTTPS JSON adapters plus a public SEC Company Facts adapter for explicit CIK/GAAP mappings, capability/secret-reference metadata, terms gating, rate pacing, retry, pagination, fallback/cache/health, timestamp normalization and deterministic provenance-bound persistence. A transparent rule version deterministically discovers themes from cited company research, retaining matched phrases, source record and evidence IDs; protected thesis history and the read-only dashboard expose those results. Provider health observations are append-only/restart-safe; an executable due-only monitor job applies approved cadence/grace contracts, persists safe failure/recovery outcomes and synchronizes durable alerts, alongside drift alerts that authenticated operators can read/acknowledge and dashboard displays. Investment policy/capital stays separate from paper OMS and active-trading budgets. Missing external job scheduling/deployment, authorized live provider deployment, richer multi-source theme models, external alert delivery, and full broader dashboard E2E coverage. Next: operational scheduler integration and broader workflow E2E. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | NOT_STARTED | ad-hoc SQLite tables only | Most required tables, migrations, immutable event records, retention and production storage choices are absent. |
| RQ-024 | Platform §23: security | PARTIAL | `security.py`, `config.py`, `audit.py`; `test_api.py`, `test_config.py` | Bearer token, in-memory rate limit and fail-closed tests exist. No server sessions/RBAC/MFA/CSRF/security headers/secret manager/SSRF controls/scans/SBOM/restore encryption. Never place secrets in frontend or source. |
| RQ-025 | Platform §§24–25: observability, backup/recovery and failure injection | PARTIAL | `observability.py`, `shadow_mode.py`, `operational_alerts.py`, `broker_sync.py`, `backup_recovery.py`; `test_observability.py`, `test_shadow_mode.py`, `test_operational_alerts.py`, `test_broker_sync.py`, `test_backup_recovery.py` | Local counters/log formatter and fixture drills exist. Alerts and failure-drill outcomes persist locally; active alerts deduplicate and retain acknowledgement/resolution transitions, and failed broker reconciliation opens a critical alert. SQLite paper records have integrity-checked, checksummed online backups and non-overwriting restore tests. No traces, SLOs, delivery integration, job monitoring, scheduled/off-site/encrypted backups, provider failover or incident procedure execution. |
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python unit tests, `.github/workflows/verify.yml`, `scripts/serve_dashboard_fixture.py` | 242 deterministic local Python tests and compilation passed on 2026-08-10. Expiry-sensitive pre-trade fixtures now use a fixed far-future timestamp rather than silently decaying with the wall clock. Direct Next production build passed, and browser E2E exercises rendered dashboard navigation, configured investment/alert and unconfigured data-health states, paper OMS evidence, and research launch success/failure through a disposable local fixture. CI now reproduces the Python checks and performs locked frontend install/build plus HTTP dashboard smoke check, but has not run on GitHub. No lint/type/security scan, load, broad failure-injection, accessibility, or full workflow E2E suite exists. |
| RQ-027 | Platform §§29–35: phased operations, reports, cost governance, runbooks, live promotion | PARTIAL | `docs/*.md` | Documentation exists but much of it repeats unverified claims. No scheduled reports/cost service/promotion controls or operational evidence. Live pilot is `REJECTED_WITH_REASON` for this build: live execution code is intentionally prohibited pending a separate explicit authorization. |
...
- Acceptance evidence: the adapter serializes same-account/day reservation checks with a transaction-scoped advisory lock; it makes reservation plus risk decision, OMS intent plus event, external fill replay/conflict handling, and validation-package/member insertion transactional. The PostgreSQL integration class seeds normalized dependencies and covers duplicate fill replay/conflict, daily-limit rejection and validation-package rollback. Local unit suite passed **262 tests** with the three PostgreSQL tests skipped because no local service was available; scoped Ruff/mypy/Bandit passed.
- Risk/security: this repository does not calculate risk, construct a strategy, call a broker, submit an order or alter the paper-only/live-disabled boundary. A write failure rolls back its transaction. Concurrent execution is designed through PostgreSQL advisory locking but has not yet been exercised against a local/CI run in this cycle.
