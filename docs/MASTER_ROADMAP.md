...
- Test evidence: offline Alembic SQL reaches migration 0005; scoped Ruff and mypy pass. The PostgreSQL integration case writes quote/halt/event/slippage/return evidence, closes the connection, reopens all authorities and checks the exact point-in-time snapshot/window. Live PostgreSQL execution is pending CI.
