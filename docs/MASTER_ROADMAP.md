...
| RQ-015 | Platform §12: strategy quality scorecard | PARTIAL | `strategy_scorecard.py`, `strategy_scorecard_v2.py`, migration `0015`; `test_strategy_scorecard*.py` | Cycle 201 adds an immutable PostgreSQL scorecard identity bound to strategy/research/dataset/feature/cost/PIT knowledge cutoffs, evidence manifest and data-health state. Normalized raw metric evidence preserves MEASURED/ASSUMED/UNAVAILABLE; transparent components and complexity penalty are explicitly non-authoritative. Eligibility fails closed without a verified manifest-bound validation package. No capacity/regime/correlation/live-consistency/signal-decay calculation or real-data evidence; no-skip CI verification remains pending. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0015`, `postgres_repositories.py`, `historical_market_data.py`, `data_health.py`, `pit_fundamentals.py`, `pit_macro.py`, `feature_authority.py`, `strategy_scorecard_v2.py`; PostgreSQL integration/restore tests | The Alembic-managed PostgreSQL authority covers normalized instruments/calendars, risk/OMS/reconciliation, immutable validation evidence, provider/source/raw-observation/dataset records, Data Health, PIT fundamentals/macro, versioned feature materializations and Cycle 201 scorecard raw evidence/package bindings. Scorecard CI/restore verification is pending; legacy SQLite application paths, complete research-table migration, object storage/queue architecture, retention operations and production deployment remain incomplete. |
