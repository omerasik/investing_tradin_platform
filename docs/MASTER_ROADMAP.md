...
| RQ-016 | Platform §13: ML baselines, model registry, explainability, drift | PARTIAL | `model_registry.py`, `model_evaluation.py`, `model_monitoring_evidence.py`, `pretrade_context.py`, migrations `0030`, `0033`; model registry/evaluation/monitoring/context tests | Cycle 224 provides chronological predecessor-relative holdout evaluation. Cycle 227 adds exact-evaluation-bound supplied scenario sensitivity plus mandatory data/feature/prediction/calibration/performance/regime/execution/cost degradation evidence under pre-approved thresholds. Immutable outcomes are non-authoritative and cannot exceed `NO_THRESHOLD_BREACH_OBSERVED`; degradation requires review. No trained baseline, actual model perturbation/invocation, permutation/SHAP, historical analogues, causal explanation, production score calculation, serving or automatic submission/approval path exists. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0035`, research/risk/operations/security/model/retrieval/evaluation/monitoring/reporting/cost/agent-governance authorities; PostgreSQL integration/restore tests | Cycle 229 run `32556281237` verifies exact job/retrieval/answer hash-bound scheduled-agent governance and all 138 restored tables. Legacy research SQLite, actual object storage/queues and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `Dockerfile`, `.github/workflows/verify.yml`, dashboard smoke and Playwright/Axe | Cycle 229 run `32556281237` passes all 451 tests without skips, all 138 restore checks and the 46-file zero-error slice. Load/performance, manual assistive-tech, registry-native OCI signing and production deployment testing remain incomplete. |
...
| RQ-016 | PARTIAL | Immutable registry and Cycle 224 holdout/calibration/local-contribution evaluation plus Cycle 227 exact-report-bound supplied sensitivity and eight-dimension degradation evidence. | Runs `32549927978` and `32553281343` prove chronology/provenance/hash gates, predecessor metrics, complete monitoring dimensions, immutable PostgreSQL restart and manual approval separation. | No trained baseline, actual perturbation, permutation/SHAP, causal explanation, historical analogues, production score computation or serving workflow; evidence is fixture-only. | RQ-004, RQ-009, RQ-012, RQ-023, RQ-025. | Serious training/evaluation needs approved data and compute budget. | Add non-executing historical-analogue/explanation evidence without model invocation. | Baselines pass leakage, calibration, robustness, explainability, drift and governance acceptance. |
...
| RQ-023 | PARTIAL | 35 Alembic migrations and 138-table restore allow-list; immutable operations, retention, identity, model/retrieval/answer/reporting/cost/agent-governance evidence. | Run `32556281237` proves migration 0035, exact governance restart/immutability and all 138 tables. | No queue/object-store byte authority, deployed scheduler/backup topology; some stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners. | Add deployment-owned scheduling/external delivery while preserving no-delete. | Production authorities and restore are migration-managed and tested. |
...
| RQ-026 | PARTIAL | 451-test suite, migration/138-table restore target and the existing full quality/security/supply-chain/frontend/browser gates. | Run `32556281237` passes 451 tests, 138 restore checks, 117/117 ratchet and zero-error 46-file slice. | Load/performance, manual assistive-tech, registry-native signing and production deployment remain absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity. | Add measured performance budgets when Chrome DevTools MCP is available. | Required suites pass without waived fixable critical findings. |
...
3. **RQ-016 model explanation:** extend Cycle 227 with non-executing historical-
   analogue evidence; keep all evidence fixture-labelled and model/tool-free.
4. **RQ-028–RQ-030 upstream adoption:** refresh identity inventory, complete
   isolated scans, then run one restricted fixture-only POC.
5. **External activation lane:** keep RQ-003–RQ-008 provider decisions concise;
