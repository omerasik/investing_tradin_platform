...
evidence is Cycle 200 (`6d9d11b`, run `31917391040`). No synthetic fixture
...
| RQ-015 | Platform §12: strategy quality scorecard | PARTIAL | `strategy_scorecard.py`, `strategy_scorecard_v2.py`, migration `0015`; `test_strategy_scorecard*.py` | Cycle 201 verifies an immutable PostgreSQL scorecard identity bound to strategy/research/dataset/feature/cost/PIT knowledge cutoffs, evidence manifest and data-health state. Normalized raw metric evidence preserves MEASURED/ASSUMED/UNAVAILABLE; transparent components and complexity penalty are explicitly non-authoritative. Eligibility fails closed without a verified manifest-bound validation package. Mainline CI run `31918548340` on `3a45028` passed the no-skip PostgreSQL and 43-table restore gates. No capacity/regime/correlation/live-consistency/signal-decay calculation or real-data evidence. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0015`, `postgres_repositories.py`, `historical_market_data.py`, `data_health.py`, `pit_fundamentals.py`, `pit_macro.py`, `feature_authority.py`, `strategy_scorecard_v2.py`; PostgreSQL integration/restore tests | The Alembic-managed PostgreSQL authority covers normalized instruments/calendars, risk/OMS/reconciliation, immutable validation evidence, provider/source/raw-observation/dataset records, Data Health, PIT fundamentals/macro, versioned feature materializations and Cycle 201 scorecard raw evidence/package bindings. Mainline CI run `31918548340` verified the scorecard migration and 43-table restore; legacy SQLite application paths, complete research-table migration, object storage/queue architecture, retention operations and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, `scripts/serve_dashboard_fixture.py` | The latest CI evidence is Cycle 200: run `31917391040` on `6d9d11b` applied migrations through `0014`, ran 304 Python tests without skips, performed the 38-table restore, and passed Ruff, mypy ratchet/critical slice, Bandit, dependency audit, SBOM, secret scan, locked frontend type/lint/audit/build and dashboard smoke gates. Local 2026-08-16 evidence is 304 tests with 25 PostgreSQL skips. Load/performance, accessibility and full workflow E2E coverage remain incomplete. |
