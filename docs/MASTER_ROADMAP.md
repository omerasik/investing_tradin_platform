...
This matrix was reconciled against `main` at `4f34e44`, the Cycle 10–15
...
local run completed 297 tests with 23 PostgreSQL-dependent skips because this
workstation has no disposable PostgreSQL DSN. The latest no-skip PostgreSQL CI
evidence remains Cycle 15 (`b67a35f`, run `31890332414`). No synthetic fixture
has been relabelled as real market-data evidence.
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0012`, `postgres_repositories.py`, `historical_market_data.py`, `data_health.py`, `pit_fundamentals.py`, `pit_macro.py`; PostgreSQL integration/restore tests | The Alembic-managed PostgreSQL authority covers normalized instruments/calendars, risk/OMS/reconciliation, immutable validation evidence, provider/source/raw-observation/dataset records, Data Health, PIT fundamentals and PIT macro records; database-side immutability and restore evidence are CI-verified. Legacy SQLite application paths, complete research-table migration, object storage/queue architecture, retention operations and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, `scripts/serve_dashboard_fixture.py` | The latest CI evidence is Cycle 15: run `31890332414` on `b67a35f` applied migrations through `0012`, ran 297 Python tests without skips, performed the 35-table restore, and passed Ruff, mypy ratchet/critical slice, Bandit, dependency audit, SBOM, secret scan, locked frontend type/lint/audit/build and dashboard smoke gates. Local 2026-08-16 evidence is 297 tests with 23 PostgreSQL skips. Load/performance, accessibility and full workflow E2E coverage remain incomplete. |
