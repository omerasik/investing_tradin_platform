...
| RQ-017 | Platform §§13.7,21: auditable AI research agents and natural-language interface | PARTIAL | `agent_research.py`, `research_retrieval.py`, `agent_answer_evaluation.py`, `api.py`, migrations `0031`–`0032`; agent/retrieval/evaluation/API tests | The non-executing multi-role ledger, structured untrusted-output validation, contradiction/safety/human-review gates and read-only query API now have Cycle 225 point-in-time retrieval plus Cycle 226 retrieval-bound answer evaluation. Exact claim citations, lexical support, utilization/diversity, evidence-bounded confidence, missing-data disclosure and causal-language blocking can yield only `BLOCKED` or `REVIEW_ELIGIBLE`. This remains deterministic fixture evidence with no model, connector, tool or action authority. No live model call, semantic/vector service, external ingestion, rich NL UX, scheduling or external-model truthfulness/safety acceptance exists. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0034`, research/risk/operations/security/model/retrieval/evaluation/monitoring/reporting/cost authorities; PostgreSQL integration/restore tests | Cycle 228 run `32554992009` verifies exact operational-job/budget hash-bound reporting and cost evidence and all 134 restored tables. Legacy research SQLite, actual object storage/queues and production deployment remain incomplete. |
...
| RQ-025 | Platform §§24–25: observability, backup/recovery and failure injection | PARTIAL | `observability.py`, `observability_sre_v2.py`, `operational_alerts.py`, `operational_jobs.py`, `governance_reporting.py`, `retention_evidence.py`, recovery modules, migrations `0021`, `0025`–`0028`, `0034`; related tests and restore drill | Cycle 228 binds report cadence to exact successful job evidence and persists supplied report/cost review evidence. No deployed scheduler, external delivery, actual object store, approved production SLO, managed telemetry, encrypted off-site backup, provider failover, IaC/deploy/rollback or staged soak exists. |
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `Dockerfile`, `.github/workflows/verify.yml`, dashboard smoke and Playwright/Axe | Cycle 228 run `32554992009` passes all 444 tests without skips, all 134 restore checks and the 45-file zero-error slice. Load/performance, manual assistive-tech, registry-native OCI signing and production deployment testing remain incomplete. |
| RQ-027 | Platform §§29–35: phased operations, reports, cost governance, runbooks, live promotion | PARTIAL | `docs/*.md`, `operational_alerts.py`, `operational_jobs.py`, `governance_reporting.py`, `backup_recovery.py`, `strategy_promotion.py`, migration `0034` | Cycle 228 adds all 13 required report contracts, five budget modes, 12 cost categories and review-only resource value/cost evidence. Reports bind exact job evidence and distinguish all five evidence classes. No scheduler, real invoice/provider ingestion, external delivery, procurement, executed external runbooks or production operations exists. Live pilot remains `REJECTED_WITH_REASON`. |
...
| RQ-017 | PARTIAL | Non-executing multi-role ledger plus immutable PIT/access-scoped lexical retrieval, retrieval-bound answer evaluation, structured adapter, safety/human review and read-only APIs. | Run `32552094284` proves exact per-claim retrieved bindings, lexical thresholds, uncertainty/disclosure gates, policy/retrieval hash binding, restart and immutability. | No semantic/vector retrieval, semantic factuality, external-model acceptance, external ingestion, scheduler or rich UX. | RQ-004–RQ-008, RQ-022–RQ-025. | Model/source terms and credentials for actual calls. | Add scheduled-workflow governance evidence without tools or execution context. | Auditable retrieval/agent workflows meet provenance, safety, evaluation and human-review gates. |
...
| RQ-023 | PARTIAL | 34 Alembic migrations and 134-table restore allow-list; immutable operations, retention, identity, model/retrieval/answer/reporting/cost evidence. | Run `32554992009` proves migration 0034 plus exact reporting/cost restart/immutability and all 134 tables. | No queue/object-store byte authority, deployed scheduler/backup topology; some stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners. | Add deployment-owned scheduling/external delivery while preserving no-delete. | Production authorities and restore are migration-managed and tested. |
...
| RQ-025 | PARTIAL | Immutable SRE/alert evidence, versioned job cadence/local routing, manifest-only retention and exact job-bound report evidence. | Runs `32293960662` and `32554992009` verify restart, rollback, retention fail-safe behavior and report/job hash bindings. | No managed telemetry, approved SLO, external paging/on-call, deployed scheduler, external delivery, actual off-site backup or soak. | RQ-002, RQ-023–RQ-027. | Cloud/on-call ownership, channels and budget. | Add a deployment-owned scheduler and separately authorized delivery adapter; retain objects until a reviewed lifecycle system exists. | Staging soak, SLOs, paging, backup/restore, rollback and failure drills meet approved targets. |
| RQ-026 | PARTIAL | 444-test suite, migration/134-table restore target and the existing full quality/security/supply-chain/frontend/browser gates. | Run `32554992009` passes 444 tests, 134 restore checks, 117/117 ratchet and zero-error 45-file slice. | Load/performance, manual assistive-tech, registry-native signing and production deployment remain absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity. | Add measured performance budgets when Chrome DevTools MCP is available. | Required suites pass without waived fixable critical findings. |
| RQ-027 | PARTIAL | Canonical roadmap, all required report/cost/value-review contracts and fail-closed review-only promotion. | Evidence is traceable through Cycle 228; live pilot remains rejected. | No deployed scheduler, real report inputs, external delivery, procurement, executed external ops, production promotion or support model. | RQ-002, RQ-015, RQ-020, RQ-023–RQ-026. | External approvals and named operators. | Add bounded agent-workflow governance or further explanation evidence while deployment remains external. | Operations are approved/exercised; live requires a new explicit decision. |
...
3. **RQ-016/RQ-017 model/agent evaluation:** extend Cycle 227 with non-executing
   historical-analogue evidence or Cycle 226 with bounded scheduled-agent-
   workflow governance; keep all evidence fixture-labelled and tool-free.
