...
| RQ-002 | Platform §4: modular event-driven architecture, FastAPI schemas, storage, queues/workflows, Docker/IaC/CI | PARTIAL | `api.py`, SQLite stores, `.github/workflows/verify.yml` | API and local SQLite only. CI now installs Python dev dependencies, compiles/tests Python, installs locked frontend dependencies, builds Next, and runs the dashboard HTTP smoke check. It has not yet run on GitHub; no Postgres/analytics/object store/cache/queue/Docker/IaC/migrations. Add local compose and migration path. |
...
| RQ-012 | Platform §11: vectorized research and realistic validation | PARTIAL | `research.py`, `strategy_validation.py`, `research_validation.py`, `strategy_promotion.py`; `test_research.py`, `test_strategy_validation.py`, `test_research_validation.py`, `test_strategy_promotion.py` | Next-period timing and simple turnover costs are tested. A launched experiment can now retain an explicit purged/embargoed, non-overlapping walk-forward protocol, held-out split/period count and compounded held-out return; incomplete, overlapping, or insufficient protocols fail closed. Purged/embargoed chronological walk-forward runs reject suffix-dependent signals. Deterministic bootstrap, Benjamini-Hochberg false-discovery control and a probabilistic-Sharpe diagnostic are tested. A durable research-only gate blocks missing reconciliation, capacity/data-quality/stress evidence and always requires human review. Capacity modelling, richer metrics, historical stress data and real provider-backed evidence remain absent. |
