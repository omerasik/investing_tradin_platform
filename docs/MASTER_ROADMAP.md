...
| RQ-016 | Platform §13: ML baselines, model registry, explainability, drift | PARTIAL | `model_registry.py`, `model_evaluation.py`, `pretrade_context.py`, migration `0030`; model registry/evaluation/context tests | Cycle 224 adds content-addressed chronological binary holdout evaluation under a policy approved before training: candidates may advance only one complexity tier, local contributions must reconcile, and precision/recall/F1, ROC/PR-AUC, Brier/log loss, information coefficient, calibration, holdout-half stability and economic value after cost are compared with a predecessor. Immutable PostgreSQL evidence can reach only `BLOCKED` or `REVIEW_ELIGIBLE`; separate human registry approval remains mandatory. No trained baseline, permutation/SHAP or causal explanation, historical-analogue/scenario sensitivity, degradation-monitoring service, model execution/serving or automatic submission path exists. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0032`, research/risk/operations/security/model/retrieval/evaluation authorities; PostgreSQL integration/restore tests | Cycle 226 run `32552094284` verifies exact policy-hash/retrieval-bound answer-evaluation evidence and all 123 restored tables. Legacy research SQLite, actual object storage/queues and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `Dockerfile`, `.github/workflows/verify.yml`, dashboard smoke and Playwright/Axe | Corrected Cycle 226 run `32552094284` passes all 429 tests without skips, all 123 restore checks and the 43-file zero-error slice. Load/performance, manual assistive-tech, registry-native OCI signing and production deployment testing remain incomplete. |
| RQ-027 | Platform §§29–35: phased operations, reports, cost governance, runbooks, live promotion | PARTIAL | `docs/*.md`, `operational_alerts.py`, `backup_recovery.py`, `strategy_promotion.py` | The roadmap and runbooks are traceable through Cycle 226; retrieval, answer-evaluation and model evidence remain non-executing and promotion is fail-closed to `REVIEW_REQUIRED`, not live. Scheduled reports/cost service, executed external runbooks and production operations remain incomplete. Live pilot remains `REJECTED_WITH_REASON`. |
...
| RQ-016 | PARTIAL | Immutable model registry plus Cycle 224's policy-bound chronological holdout/calibration/local-contribution evaluator and drift-triggered disable/reapproval. | PR-head run `32549927978` proves immediate complexity progression, chronology/provenance/hash gates, predecessor-relative metrics, immutable PostgreSQL restart and manual approval separation. | No trained baseline, permutation/SHAP or causal explanation, historical analogues/scenario sensitivity, production degradation monitor or serving workflow; evidence is fixture-only. | RQ-004, RQ-009, RQ-012, RQ-023, RQ-025. | Serious training/evaluation needs approved data and compute budget. | Add non-executing explanation sensitivity and degradation-evidence contracts, preserving fixture labels until lawful data exists. | Baselines pass leakage, calibration, robustness, explainability, drift and governance acceptance. |
...
| RQ-023 | PARTIAL | 32 Alembic migrations and 123-table restore allow-list; immutable operations, retention, identity, model, retrieval and answer-evaluation evidence. | Run `32552094284` proves migration 0032 plus evaluator restart/immutability and all 123 tables. | No queue/object-store byte authority, deployed scheduler/backup topology; some stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners. | Add deployment-owned scheduling/external delivery while preserving no-delete. | Production authorities and restore are migration-managed and tested. |
...
| RQ-026 | PARTIAL | 429-test suite, migration/123-table restore target and the existing full quality/security/supply-chain/frontend/browser gates. | Run `32552094284` passes 429 tests, 123 restore checks, 117/117 ratchet and zero-error 43-file slice. | Load/performance, manual assistive-tech, registry-native signing and production deployment remain absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity. | Add measured performance budgets when Chrome DevTools MCP is available. | Required suites pass without waived fixable critical findings. |
| RQ-027 | PARTIAL | Canonical roadmap, governance/runbooks and fail-closed review-only promotion. | Evidence is traceable through Cycle 226; live pilot remains rejected. | No scheduled reports/cost service, executed external ops, production promotion or support model. | RQ-002, RQ-015, RQ-020, RQ-023–RQ-026. | External approvals and named operators. | Add scheduled governance-report and cost-accounting contracts. | Operations are approved/exercised; live requires a new explicit decision. |
...
3. **RQ-016/RQ-017 model/agent evaluation:** extend Cycle 224 with explanation-
   sensitivity/degradation evidence and Cycle 226 with scheduled-workflow
   governance evidence; keep both non-executing and fixture-labelled.
