...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0030`, research/risk/operations/security/model-evaluation authorities; PostgreSQL integration/restore tests | Cycle 224 PR-head run `32549927978` verifies migration 0030, immutable model-evaluation policy/report/observation evidence and all 116 restored tables. Legacy research SQLite, actual object storage/queues and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `Dockerfile`, `.github/workflows/verify.yml`, dashboard smoke and Playwright/Axe | Cycle 224 PR-head run `32549927978` passes all 417 tests without skips, all 116 restore checks, the 41-file zero-error slice, signed archive provenance/SBOM verification and every existing gate. The full report retains 26 vendor-unfixed HIGH/CRITICAL findings and zero fixable ones. Load/performance, manual assistive-tech, registry-native OCI signing and production deployment testing remain incomplete. |
| RQ-027 | Platform §§29–35: phased operations, reports, cost governance, runbooks, live promotion | PARTIAL | `docs/*.md`, `operational_alerts.py`, `backup_recovery.py`, `strategy_promotion.py` | The roadmap and runbooks are traceable through Cycle 224 and distinguish verified provider-neutral evidence from external activation blockers; model evaluation reaches review eligibility only and promotion is fail-closed to `REVIEW_REQUIRED`, not live. Scheduled operational reports/cost service, external provider runbooks executed under authorization, and production operations remain incomplete. Live pilot remains `REJECTED_WITH_REASON`: live execution is prohibited pending separate explicit authorization. |
...
| RQ-017 | PARTIAL | Non-executing multi-role research ledger, structured adapter, contradiction/safety/human review and read-only APIs. | Agent and API tests verify source authorization and order-style text rejection. | No retrieval service, scheduler, rich UX or external-model truthfulness/safety evaluation. | RQ-004–RQ-008, RQ-022–RQ-025. | Model/source terms and credentials for actual calls. | Add retrieval-evidence and scheduled-workflow authorities without tools or execution context. | Auditable retrieval/agent workflows meet provenance, safety, evaluation and human-review gates. |
...
| RQ-023 | PARTIAL | 30 Alembic migrations and 116-table restore allow-list; immutable job/run/local-outbox, retention, identity-security and model-evaluation evidence; SQLite retained for local research. | PR-head run `32549927978` proves migration 0030, evaluation policy/report/observation restart and immutability plus all 116 restore-critical tables. | No queue/object-store byte authority, lifecycle executor, deployed scheduler/backup topology; some research stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners for production. | Add deployment-owned scheduling/external delivery while preserving the separate no-delete boundary. | Production authorities, lifecycle/retention, backups and restore are migration-managed and tested. |
...
| RQ-026 | PARTIAL | 417-test suite, migration/116-table restore target, Ruff/mypy/Bandit/audit/SBOM/secrets, hardened/scanned/attested research archive, locked frontend build, strict nonce-CSP smoke and Playwright/Axe. | PR-head run `32549927978` passes all 417 tests, 116 restore checks, 117/117 mypy ratchet, zero-error 41-file slice, two signed/verified attestations and every existing gate. | Twenty-six vendor-unfixed HIGH/CRITICAL findings remain; load/performance, manual assistive-tech, registry-native OCI signing, production deployment and broader chaos tests are absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity for production-grade load tests. | Add measured performance-budget validation; the Chrome DevTools MCP is currently unavailable locally. | Required functional, security, performance, recovery and deployment suites pass without waived fixable critical findings. |
| RQ-027 | PARTIAL | Canonical roadmap, governance/runbooks and fail-closed review-only promotion. | CI/evidence records are traceable through Cycle 224; live pilot remains rejected. | No scheduled reports/cost service, executed external ops, production promotion or support model. | RQ-002, RQ-015, RQ-020, RQ-023–RQ-026. | Provider/cloud/broker approvals, budget, named operators and separate live authorization. | Add scheduled governance-report and cost-accounting contracts. | Operational reports, costs, runbooks and staged paper promotion are owner-approved and exercised; live requires a new explicit decision. |
...
3. **RQ-016/RQ-017 model/agent evaluation:** extend Cycle 224's chronological
   evaluator with explanation-sensitivity/degradation evidence and add retrieval
   evidence; keep both non-executing and fixture-labelled.
