...
- Test evidence: existing paper-runtime tests remain green locally. A new configured PostgreSQL integration case seeds the reviewed authorities, reconciled account and point-in-time inputs, then requires assessment approval, one daily reservation, one keyed assessment and one acknowledged paper OMS intent. Live PostgreSQL execution is pending CI.
