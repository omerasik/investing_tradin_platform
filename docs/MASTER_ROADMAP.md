...
| RQ-007 | Platform §5.5: news/events intelligence | IN_PROGRESS | `market_intelligence.py`, `news_event_intelligence.py`, migration `0020`; news/event tests | Cycle 206 implements a PostgreSQL-native, correction-aware, rights-gated research evidence authority with deterministic dedupe/clustering, professional-instrument links, event taxonomy, credibility/uncertainty, point-in-time replacement and confidence withdrawal. Hosted verification remains pending; no provider is activated. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, dashboard smoke | Exact-merge run `31922415005` closed Cycle 205. Cycle 206 local verification discovers 340 tests with 31 PostgreSQL-only skips and passes all runnable tests, compileall, Ruff, the unchanged 120/120 mypy debt ratchet, a zero-error 25-file critical slice and Bandit. Hosted migration/restart/81-table restore and the remaining configured gates are pending. |
