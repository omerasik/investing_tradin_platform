...
| RQ-002 | Platform §4: modular event-driven architecture, FastAPI schemas, storage, queues/workflows, Docker/IaC/CI | PARTIAL | `persistence.py`, `postgres_schema.py`, `migrations/`, `Dockerfile`, `.github/workflows/verify.yml` | Cycle 219 CI verifies a digest-pinned, exact-dependency, non-root local-research API image under a read-only/capability-free/no-new-privileges runtime. PostgreSQL schema/backfill/restore gates remain verified. The image is not the PostgreSQL deployment: queue/cache/actual object storage, signing/provenance, container CVE scan, registry, IaC, orchestration and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `Dockerfile`, `.github/workflows/verify.yml`, dashboard smoke and Playwright/Axe | Cycle 219 PR-head run `32298849742` passes all 393 tests without skips, all 111 restore checks, the 117/117 mypy ratchet, zero-error 39-file slice, hardened non-root research-container runtime and every existing gate. Load/performance, manual assistive-tech, image provenance/CVE and production deployment testing remain incomplete. |
| RQ-027 | Platform §§29–35: phased operations, reports, cost governance, runbooks, live promotion | PARTIAL | `docs/*.md`, `operational_alerts.py`, `backup_recovery.py`, `strategy_promotion.py` | The roadmap and runbooks are traceable through Cycle 219 and distinguish verified provider-neutral evidence from external activation blockers; promotion is fail-closed to `REVIEW_REQUIRED`, not live. Scheduled operational reports/cost service, external provider runbooks executed under authorization, and production operations remain incomplete. Live pilot remains `REJECTED_WITH_REASON`: live execution is prohibited pending separate explicit authorization. |
...
| RQ-002 | PARTIAL | Modular Python/FastAPI/Next monolith, PostgreSQL/Alembic, append-only authorities, CI and a narrow non-root research API image. | Cycle 219 run `32298849742` proves digest-pinned build, UID/GID 10001, read-only/capability-free/no-new-privileges runtime, paper-only health and authorization probes. | Image is SQLite local-research only; no queue/cache/object-store authority, signing/provenance/CVE scan, registry, IaC/orchestration or deployed PostgreSQL environment. | RQ-023–RQ-027. | Cloud account, region, budget and ownership are needed only for deployment. | Add artifact provenance/scanning and a disposable PostgreSQL migration/health/rollback contract. | Reproducible staging deployment passes migration, health, rollback, security and provenance gates. |
...
| RQ-026 | PARTIAL | 393-test suite, migration/111-table restore target, Ruff/mypy/Bandit/audit/SBOM/secrets, hardened non-root research image, locked frontend build, smoke and Playwright/Axe. | PR-head run `32298849742` passes all 393 tests, 111 restore checks, 117/117 mypy ratchet, zero-error 39-file slice, actual hardened container runtime and every existing gate. | Load/performance, manual assistive-tech, image provenance/CVE, production deployment and broader chaos tests are absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity for production-grade load tests. | Add image provenance/CVE and performance-budget validation; the Chrome DevTools MCP is currently unavailable locally. | Required functional, security, performance, recovery and deployment suites pass without waived critical findings. |
| RQ-027 | PARTIAL | Canonical roadmap, governance/runbooks and fail-closed review-only promotion. | CI/evidence records are traceable through Cycle 219; live pilot remains rejected. | No scheduled reports/cost service, executed external ops, production promotion or support model. | RQ-002, RQ-015, RQ-020, RQ-023–RQ-026. | Provider/cloud/broker approvals, budget, named operators and separate live authorization. | Add scheduled governance-report and cost-accounting contracts. | Operational reports, costs, runbooks and staged paper promotion are owner-approved and exercised; live requires a new explicit decision. |
...
2. **RQ-024/RQ-026 security and deployment quality:** external-session and
   auditable claim-mapping boundaries, nonce-based CSP, image provenance/CVE
   scanning and measured performance budgets; retain manual assistive-technology
   acceptance as a deployment gate. The non-root research-container check is
   now CI-verified, but it is not a production deployment.
...
- This is a SQLite-backed local-research artifact, not a production/PostgreSQL
  deployment. Image signing/provenance, container CVE scanning, registry policy,
  IaC/orchestration, network/TLS policy, rollback and staging soak remain open.
