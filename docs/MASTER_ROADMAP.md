...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0018`, Trend/Feature/Data Health/scorecard/validation/investment/regime authorities; PostgreSQL integration/restore tests | Migration 0018 adds content-addressed regime models/runs, reusable observations/evaluations with immutable run memberships, Data Health bindings and review-only risk candidates. Cycle 203 mainline CI run `31920718692` hash-matched 54 restored critical tables; the Cycle 204 restore set expands to 62 and awaits hosted CI. Legacy SQLite composition, object storage/queues, retention and production deployment remain incomplete. |
...
## Cycle 204 evidence — Regime Engine V2 (in progress)
...
- Local verification discovered **328 tests**, passed all runnable tests with
  ...
  **23-file** critical slice, and the sole Alembic head 0018. Hosted PostgreSQL
  migration/replay/restart/immutability/restore evidence remains required.
