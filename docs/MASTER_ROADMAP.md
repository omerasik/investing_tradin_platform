...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0033`, research/risk/operations/security/model/retrieval/evaluation/monitoring authorities; PostgreSQL integration/restore tests | Cycle 227 run `32553281343` verifies exact policy/evaluation-hash-bound model monitoring evidence and all 127 restored tables. Legacy research SQLite, actual object storage/queues and production deployment remain incomplete. |
...
| RQ-025 | Platform §§24–25: observability, backup/recovery and failure injection | PARTIAL | `observability.py`, `observability_sre_v2.py`, `operational_alerts.py`, `operational_jobs.py`, `retention_evidence.py`, recovery modules, migrations `0021`, `0025`–`0028`; related tests and restore drill | Cycle 216 run `32293960662` adds immutable manifest-only retention evidence and injected evaluation rollback on top of Cycle 215 job/alert/outbox evidence. No deployed scheduler, external delivery, actual object store, approved production SLO, managed telemetry, encrypted off-site backup, provider failover, IaC/deploy/rollback or staged soak exists. |
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `Dockerfile`, `.github/workflows/verify.yml`, dashboard smoke and Playwright/Axe | Cycle 227 run `32553281343` passes all 435 tests without skips, all 127 restore checks and the 44-file zero-error slice. Load/performance, manual assistive-tech, registry-native OCI signing and production deployment testing remain incomplete. |
| RQ-027 | Platform §§29–35: phased operations, reports, cost governance, runbooks, live promotion | PARTIAL | `docs/*.md`, `operational_alerts.py`, `backup_recovery.py`, `strategy_promotion.py` | The roadmap and runbooks are traceable through Cycle 227; model/retrieval/answer/monitoring evidence remains non-executing and promotion is fail-closed to `REVIEW_REQUIRED`, not live. Scheduled reports/cost service, executed external runbooks and production operations remain incomplete. Live pilot remains `REJECTED_WITH_REASON`. |
...
| RQ-023 | PARTIAL | 33 Alembic migrations and 127-table restore allow-list; immutable operations, retention, identity, model evaluation/monitoring, retrieval and answer evidence. | Run `32553281343` proves migration 0033 plus monitoring restart/immutability and all 127 tables. | No queue/object-store byte authority, deployed scheduler/backup topology; some stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners. | Add deployment-owned scheduling/external delivery while preserving no-delete. | Production authorities and restore are migration-managed and tested. |
...
| RQ-025 | PARTIAL | Immutable SRE/alert evidence, versioned job cadence/local routing and manifest-only retention/object lifecycle evidence. | Cycle 216 CI verifies policy/manifest/evaluation idempotency, restart, rollback, legal-hold/disabled fail-safe behavior and no deletion state. | No managed telemetry, approved SLO, external paging/on-call, deployed scheduler, external delivery, actual off-site backup or soak. | RQ-002, RQ-023–RQ-027. | Cloud/on-call ownership, channels and budget. | Add a deployment-owned scheduler and separately authorized delivery adapter; retain objects until a reviewed lifecycle system exists. | Staging soak, SLOs, paging, backup/restore, rollback and failure drills meet approved targets. |
| RQ-026 | PARTIAL | 435-test suite, migration/127-table restore target and the existing full quality/security/supply-chain/frontend/browser gates. | Run `32553281343` passes 435 tests, 127 restore checks, 117/117 ratchet and zero-error 44-file slice. | Load/performance, manual assistive-tech, registry-native signing and production deployment remain absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity. | Add measured performance budgets when Chrome DevTools MCP is available. | Required suites pass without waived fixable critical findings. |
| RQ-027 | PARTIAL | Canonical roadmap, governance/runbooks and fail-closed review-only promotion. | Evidence is traceable through Cycle 227; live pilot remains rejected. | No scheduled reports/cost service, executed external ops, production promotion or support model. | RQ-002, RQ-015, RQ-020, RQ-023–RQ-026. | External approvals and named operators. | Add scheduled governance-report and cost-accounting contracts. | Operations are approved/exercised; live requires a new explicit decision. |
...
3. **RQ-016/RQ-017 model/agent evaluation:** extend Cycle 227 with non-executing
   historical-analogue or scheduled-governance evidence and Cycle 226 with
   scheduled-workflow governance; keep all evidence fixture-labelled.
