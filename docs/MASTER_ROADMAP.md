...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0025`, research/risk/operations authorities; PostgreSQL integration/restore tests | Cycle 213 PR-head run `32288130114` verifies migration 0025, immutable alert events and all 102 restored tables. Legacy research SQLite, object storage/queues, retention and production deployment remain incomplete. |
...
| RQ-025 | Platform §§24–25: observability, backup/recovery and failure injection | PARTIAL | `observability.py`, `observability_sre_v2.py`, `shadow_mode.py`, `operational_alerts.py`, `backup_recovery.py`, `postgres_recovery.py`, migrations `0021`, `0025`; related tests and restore drill | Cycle 207 verifies SRE evidence; Cycle 213 adds atomic risk-alert persistence, immutable transitions and an injected-failure rollback drill in run `32288130114`. External routing, approved production SLO attainment, managed telemetry, scheduler monitoring, encrypted off-site backups, provider failover, IaC/deploy/rollback and staged soak remain incomplete. |
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, dashboard smoke and Playwright | Cycle 213 PR-head run `32288130114` passes all 367 tests without skips, all 102 restore checks, the 117/117 mypy ratchet, zero-error 35-file slice and every configured workflow gate. Performance/load, comprehensive accessibility, container and deployment testing remain incomplete. |
...
| RQ-019 | PARTIAL | Deterministic individual/portfolio risk, scoped kill switches, PG reservations/decisions, policy-bound stop/gap loss and atomic rejection alerts. | PR-head run `32288130114` verifies portfolio rejection without reservation, kill-switch severity, concurrent replay, restart and injected alert rollback. | Gap buffer is policy-prescribed rather than empirically calibrated; snapshot/reconciliation reads, cancellation behavior, broad producers and external delivery remain incomplete. | RQ-003–RQ-004, RQ-011, RQ-018, RQ-020, RQ-023. | Calibration/live sources only; deterministic extensions are unblocked. | Add execution-quality and operational shadow evidence without a network broker. | Complete limits/stress/kill switches are atomic, explainable and failure-tested under concurrency. |
| RQ-020 | PARTIAL | Paper OMS/broker abstraction, idempotent lifecycle/fills, reconciliation, checked submission and protected read workspace. | Configured PostgreSQL runtime executes simulated approval-to-fill/restart and rejects risk increases before reconciliation. | No real broker sandbox, measured execution quality or operational shadow campaign. | RQ-018, RQ-019, RQ-023–RQ-026. | Broker sandbox account, credentials and operator-approved terms. | Add provider-neutral execution-quality and shadow evidence before activation. | Approved sandbox completes soak, reconciliation, failure recovery and quality thresholds; live remains separately gated. |
...
1. **RQ-020 paper operations:** execution-quality and operational shadow
   evidence without a network broker; Cycle 213 closes the bounded atomic
   risk-violation alert and concurrency/failure slice.
2. **RQ-023/RQ-025 durable operations:** job/retention/object evidence,
   scheduler monitoring and alert-routing boundaries.
3. **RQ-024/RQ-026 security and deployment quality:** role/session abstractions,
   headers, accessibility, performance budgets and non-root container checks.
4. **RQ-016/RQ-017 model/agent evaluation:** holdout/calibration/explainability
   and retrieval evidence, still non-executing and fixture-labelled.
5. **RQ-028–RQ-030 upstream adoption:** refresh identity inventory, complete
   isolated scans, then run one restricted fixture-only POC.
6. **External activation lane:** keep RQ-003–RQ-008 provider decisions concise;
