...
- Test evidence: the new PostgreSQL integration case applies the migration, writes and reopens policy/assessment evidence, verifies identical replay, checks wrong-key rejection and proves database UPDATE rejection. Local composition tests, offline Alembic generation, scoped Ruff and mypy pass. Live PostgreSQL execution remains pending CI for this commit.
...
- Status: IMPLEMENTED/TESTED_LOCAL; not yet VERIFIED.
