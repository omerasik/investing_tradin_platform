...
| RQ-002 | Platform §4: modular event-driven architecture, FastAPI schemas, storage, queues/workflows, Docker/IaC/CI | PARTIAL | `persistence.py`, `postgres_schema.py`, `migrations/`, `.github/workflows/verify.yml` | An Alembic-managed normalized PostgreSQL schema covers major domain identities, provenance, financial NUMERIC values, immutable evidence triggers and key indexes. CI verifies mapped backfill APPLY/replay/conflict behavior, an ephemeral PostgreSQL service, all 91 restored tables and the configured Python/security/frontend/browser gates. Unsupported legacy surfaces fail closed. Queue/cache/object storage, a non-root container, IaC and production deployment remain incomplete. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0027`, research/risk/operations authorities; PostgreSQL integration/restore tests | Cycle 215 PR-head run `32292245989` verifies migration 0027, immutable job policy/run/local-outbox evidence and all 108 restored tables. Legacy research SQLite, object storage/queues, retention and production deployment remain incomplete. |
...
| RQ-025 | Platform §§24–25: observability, backup/recovery and failure injection | PARTIAL | `observability.py`, `observability_sre_v2.py`, `operational_alerts.py`, `operational_jobs.py`, recovery modules, migrations `0021`, `0025`–`0027`; related tests and restore drill | Cycle 215 run `32292245989` adds immutable job/run evidence, overdue/recovery alerts, local outbox routing and injected outbox rollback. No deployed scheduler, external delivery, approved production SLO, managed telemetry, encrypted off-site backup, provider failover, IaC/deploy/rollback or staged soak exists. |
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, dashboard smoke and Playwright | Cycle 215 PR-head run `32292245989` passes all 377 tests without skips, all 108 restore checks, the 117/117 mypy ratchet, zero-error 37-file slice and every configured workflow gate. Performance/load, comprehensive accessibility, container and deployment testing remain incomplete. |
...
| RQ-023 | PARTIAL | 27 Alembic migrations and 108-table restore allow-list; immutable job/run/local-outbox evidence; SQLite retained for local research. | PR-head run `32292245989` proves migration 0027, rollback/restart/immutability and all 108 restore-critical tables. | No queue/object-store/retention authority or deployed scheduler/backup topology; some research stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners for production. | Define retention/object evidence and classify remaining SQLite surfaces. | Production authorities, lifecycle/retention, backups and restore are migration-managed and tested. |
...
| RQ-025 | PARTIAL | Immutable SRE/alert evidence plus versioned job cadence, terminal runs, overdue/recovery monitoring and local-outbox routing. | Cycle 215 CI verifies idempotency, restart, monotonic recovery and alert/outbox rollback without executing jobs or delivery. | No managed telemetry, approved SLO, external paging/on-call, deployed scheduler, external delivery, off-site backup or soak. | RQ-002, RQ-023–RQ-027. | Cloud/on-call ownership, channels and budget. | Add retention/object evidence, then a deployment-owned scheduler/authorized delivery adapter. | Staging soak, SLOs, paging, backup/restore, rollback and failure drills meet approved targets. |
| RQ-026 | PARTIAL | 377-test suite, migration/108-table restore target, Ruff/mypy/Bandit/audit/SBOM/secrets, locked frontend build, smoke and Playwright. | PR-head run `32292245989` passes all 377 tests, 108 restore checks, 117/117 mypy ratchet, zero-error 37-file slice and every configured gate. | Load/performance, comprehensive accessibility, container/deployment and broader chaos tests are absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity for production-grade load tests. | Add accessibility, performance-budget and container validation gates. | Required functional, security, performance, recovery and deployment suites pass without waived critical findings. |
| RQ-027 | PARTIAL | Canonical roadmap, governance/runbooks and fail-closed review-only promotion. | CI/evidence records are traceable through Cycle 214; live pilot remains rejected. | No scheduled reports/cost service, executed external ops, production promotion or support model. | RQ-002, RQ-015, RQ-020, RQ-023–RQ-026. | Provider/cloud/broker approvals, budget, named operators and separate live authorization. | Add scheduled governance-report and cost-accounting contracts. | Operational reports, costs, runbooks and staged paper promotion are owner-approved and exercised; live requires a new explicit decision. |
...
1. **RQ-023/RQ-025 durable operations:** retention/object evidence, followed by
   a deployment-owned scheduler and authorized external-delivery adapter.
