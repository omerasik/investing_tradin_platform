...
| RQ-013 | Platform §§11.1–11.2: high-fidelity event simulation | PARTIAL | `event_backtest.py`, `cross_engine.py`, `paper_execution.py`, `corporate_actions.py`; `test_event_backtest.py`, `test_cross_engine.py`, `test_paper_execution.py`, `test_corporate_actions.py` | Deterministic event engine is exercised for latency, sessions, bid/ask, market/limit/stop/stop-limit, partial fills, fee, square-root impact, cash/position accounting and effective one-time corporate-action application. An append-only golden artifact now drives an immutable vector signal sequence through same-timestamp single-price close-auction orders, event fills and point-in-time split/cash-dividend handling, then retains final equity/exposure reconciliation. Funding/borrow/margin remain separate helpers; no futures/FX/tax/queue model or realistic multi-price golden artifact exists. |
| RQ-014 | Upstream §36.10: independent cross-engine validation | PARTIAL | `cross_engine.py`, `research.py`, `strategy_promotion.py`; `test_cross_engine.py`, `test_research.py`, `test_strategy_promotion.py` | Final-equity/fill/position and fill identity/timestamp/quantity/price/fee mismatch tests pass. Every newly launched vectorized baseline experiment compares final equity and turnover against a separate iterative close-to-close accounting implementation. A durable golden artifact also runs one immutable adjusted-close dataset/signal sequence through explicit close-auction event orders, fills and point-in-time split/dividend actions, then compares final event equity/exposure. A research-only promotion gate blocks failed reconciliation. No three-engine run, spread/latency/partial-fill golden scenario, event artifact API, or full corporate-action comparison exists. |
