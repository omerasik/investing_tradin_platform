...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0029`, research/risk/operations/security authorities; PostgreSQL integration/restore tests | Cycle 222 PR-head run `32547343608` verifies migration 0029, immutable identity mapping/authorization evidence and all 113 restored tables. Legacy research SQLite, actual object storage/queues and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `Dockerfile`, `.github/workflows/verify.yml`, dashboard smoke and Playwright/Axe | Cycle 223 PR-head run `32548653129` passes all 411 tests without skips, all 113 restore checks, strict nonce-CSP production/browser evidence, signed archive provenance/SBOM verification and every existing gate. The full report retains 26 vendor-unfixed HIGH/CRITICAL findings and zero fixable ones. Load/performance, manual assistive-tech, registry-native OCI signing and production deployment testing remain incomplete. |
| RQ-027 | Platform §§29–35: phased operations, reports, cost governance, runbooks, live promotion | PARTIAL | `docs/*.md`, `operational_alerts.py`, `backup_recovery.py`, `strategy_promotion.py` | The roadmap and runbooks are traceable through Cycle 223 and distinguish verified provider-neutral evidence from external activation blockers; promotion is fail-closed to `REVIEW_REQUIRED`, not live. Scheduled operational reports/cost service, external provider runbooks executed under authorization, and production operations remain incomplete. Live pilot remains `REJECTED_WITH_REASON`: live execution is prohibited pending separate explicit authorization. |
...
| RQ-016 | PARTIAL | Immutable model registry, approvals, calibrated prediction schema and drift-triggered disable/reapproval. | Registry/context tests verify missing approval, provenance or drift evidence blocks. | No trained baseline, evaluator, explainability suite, monitoring service or serving workflow. | RQ-004, RQ-009, RQ-012, RQ-023, RQ-025. | Serious training/evaluation needs approved data and compute budget. | Build a provider-neutral holdout/calibration/explainability evaluator with fixture labels. | Baselines pass leakage, calibration, robustness, explainability, drift and governance acceptance. |
...
3. **RQ-016/RQ-017 model/agent evaluation:** holdout/calibration/explainability
   and retrieval evidence, still non-executing and fixture-labelled.
