...
| RQ-008 | Platform §5.6: lawful social/narrative intelligence | PARTIAL | `social_narrative_intelligence.py`, migration `0022`; focused unit/PostgreSQL tests | Cycle 210 locally tests a provider-neutral, rights/privacy-gated, immutable PIT evidence core. It separates all six required discussion classes and calculates volume/acceleration, unique-author growth, bot/spam/coordination/pump risk, sentiment/change/disagreement/emotion, narrative clustering/emergence/persistence, influencer/language/geography distributions and price divergence. Evidence is synthetic, research-only, cannot be a standalone trade trigger and has no automatic/signal/order authority. PostgreSQL migration/restart/97-table restore remain pending hosted CI; no provider is activated. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0022`, research/risk/operations authorities; PostgreSQL integration/restore tests | Migration 0022 adds six immutable social/narrative authority tables and expands the restore allow-list from 91 to 97; hosted migration/restore evidence is pending Cycle 210 CI. Legacy research SQLite, object storage/queues, retention and production deployment remain incomplete. |
...
| RQ-008 | PARTIAL | Provider-neutral rights/privacy policy, sanitized content observations, deterministic narrative clusters, full metric windows, Data Health binding and PIT PostgreSQL store. | Five focused unit tests pass locally; full local run discovers 357 tests and passes 323 with 34 explicit PostgreSQL skips. Ruff, compileall and the new zero-error mypy slice pass. | No lawful real connector/corpus, measured bot/topic classifier, multilingual evaluation, production cadence, feature integration or operator workspace; PostgreSQL evidence awaits hosted CI. | RQ-003, RQ-004, RQ-007, RQ-009, RQ-022, RQ-023. | Real-source activation needs approved terms, credentials, privacy review, universe and labeled evaluation corpus. | Complete hosted PostgreSQL/restart/97-table restore verification, then expose read-only narrative evidence and integrate only authorized derived features. | Immutable PIT aggregates distinguish all required classes/metrics, detect manipulation/crowding, preserve rights/privacy/provenance, meet labeled quality thresholds and never independently trigger a trade. |
...
| RQ-023 | PARTIAL | 22 Alembic migrations and 97-table restore allow-list; SQLite retained for local research. | The prior 91-table exact-mainline restore passes in run `32278879487`; migration 0022 and all 97 tables await Cycle 210 hosted CI. | No queue/object-store/job/retention authority or deployed backup topology; some research stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners for production. | Verify migration 0022 and all 97 tables, then define durable job/retention/object evidence. | Production authorities, lifecycle/retention, backups and restore are migration-managed and tested. |
...
| RQ-026 | PARTIAL | 357-test branch suite, migration/restore, Ruff/mypy/Bandit/audit/SBOM/secrets, locked frontend build, smoke and Playwright. | Exact-mainline run `32278879487` passed the prior 351-test/91-table baseline; Cycle 210's six new tests and 97-table restore await hosted CI. | Load/performance, comprehensive accessibility, container/deployment and broader chaos tests are absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity for production-grade load tests. | Verify Cycle 210, then add accessibility, performance-budget and container validation gates. | Required functional, security, performance, recovery and deployment suites pass without waived critical findings. |
...
1. **Cycle 210 verification — RQ-008 social/narrative core:** complete hosted
   PostgreSQL migration, immutability, PIT restart and 97-table restore evidence.
2. **RQ-018/RQ-022 signal operations:** expiry scheduling, reason APIs, Signal
   Explorer and accessibility evidence.
3. **RQ-019/RQ-020 risk and paper operations:** per-trade loss/stop-gap,
   execution-quality and shadow evidence without a network broker.
4. **RQ-023/RQ-025 durable operations:** job/retention/object evidence,
   scheduler monitoring and alert-routing boundaries.
5. **RQ-024/RQ-026 security and deployment quality:** role/session abstractions,
   headers, accessibility, performance budgets and non-root container checks.
6. **RQ-016/RQ-017 model/agent evaluation:** holdout/calibration/explainability
   and retrieval evidence, still non-executing and fixture-labelled.
7. **RQ-028–RQ-030 upstream adoption:** refresh identity inventory, complete
   isolated scans, then run one restricted fixture-only POC.
8. **External activation lane:** keep RQ-003–RQ-007 provider decisions concise;
...
## Cycle 210 evidence — social/narrative intelligence core (TESTED LOCALLY)
...
- Migration 0022 adds six immutable tables and expands fresh-restore comparison
  ...
  activation counts; it awaits hosted CI because no local DSN exists.
- Local verification discovers **357 tests**, passes **323** and explicitly
  ...
  and a zero-error new-module slice pass. Real-source activation, measured
  classifier quality, feature integration and operator UI remain incomplete.
