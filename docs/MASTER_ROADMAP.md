...
| RQ-007 | Platform §5.5: news/events intelligence | PARTIAL | `market_intelligence.py`, `news_event_intelligence.py`, migration `0020`; news/event tests | PR CI run `31922911169` verifies a PostgreSQL-native, correction-aware, rights-gated research evidence authority with deterministic dedupe/clustering, professional-instrument links, event taxonomy, credibility/uncertainty, point-in-time replacement and confidence withdrawal. Exact-merge verification remains pending; no provider is activated. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0020`, Trend/Feature/Data Health/scorecard/validation/investment/regime/portfolio/news authorities; PostgreSQL integration/restore tests | Cycle 205 exact-merge run `31922415005` hash-matched all 71 prior restored critical tables. Migration 0020 adds ten immutable news source/document/link/extraction/cluster/lineage/assessment/candidate tables; PR CI run `31922911169` hash-matched all 81 restored critical tables. Legacy SQLite composition, object storage/queues, retention and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, dashboard smoke | Exact-merge run `31922415005` closed Cycle 205. Cycle 206 PR CI run `31922911169` applies migration 0020, runs all 340 tests without skips, matches all 81 restored tables, and passes Ruff, the unchanged 120/120 mypy debt ratchet, a zero-error 25-file critical slice, Bandit, dependency audits, SBOM, secret scan, TypeScript, ESLint, Next build and dashboard smoke. Exact-merge evidence remains pending. |
...
## Cycle 206 evidence — News/Event Intelligence Core (PR-verified)
