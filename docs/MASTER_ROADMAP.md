...
- Test evidence: offline Alembic SQL reaches migration 0006; scoped Ruff and mypy pass. The PostgreSQL integration case writes an instrument/profile/session, a fully validated proposal and reviewed model, closes the connection and verifies exact recovery and approval after restart. Live PostgreSQL execution is pending CI.
