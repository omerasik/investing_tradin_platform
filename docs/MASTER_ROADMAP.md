...
| RQ-002 | Platform §4: modular event-driven architecture, FastAPI schemas, storage, queues/workflows, Docker/IaC/CI | PARTIAL | `persistence.py`, `postgres_schema.py`, `migrations/`, `.github/workflows/verify.yml` | An Alembic-managed normalized PostgreSQL schema covers major domain identities, provenance, financial NUMERIC values, immutable evidence triggers and key indexes. CI verifies mapped backfill APPLY/replay/conflict behavior, an ephemeral PostgreSQL service, all 111 restored tables and the configured Python/security/frontend/browser gates. Unsupported legacy surfaces fail closed. Queue/cache/actual object storage, a non-root container, IaC and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, dashboard smoke and Playwright/Axe | Cycle 218 PR-head run `32297293629` passes all 388 tests without skips, all 111 restore checks, the 117/117 mypy ratchet, zero-error 39-file slice and every configured dependency, security, build and browser gate. Load/performance, manual assistive-tech, container and deployment testing remain incomplete. |
| RQ-027 | Platform §§29–35: phased operations, reports, cost governance, runbooks, live promotion | PARTIAL | `docs/*.md`, `operational_alerts.py`, `backup_recovery.py`, `strategy_promotion.py` | The roadmap and runbooks are traceable through Cycle 218 and distinguish verified provider-neutral evidence from external activation blockers; promotion is fail-closed to `REVIEW_REQUIRED`, not live. Scheduled operational reports/cost service, external provider runbooks executed under authorization, and production operations remain incomplete. Live pilot remains `REJECTED_WITH_REASON`: live execution is prohibited pending separate explicit authorization. |
...
| RQ-002 | PARTIAL | Modular Python/FastAPI/Next monolith, PostgreSQL/Alembic, append-only authorities and CI. | Migration 0021, 91-table restore, build and browser gates passed in run `32278879487`. | No queue/cache/object-store authority, container image, IaC or deployed environment. | RQ-023–RQ-027. | Cloud account, region, budget and ownership are needed only for deployment. | Define a non-root reproducible container and disposable migration/health validation. | Reproducible local/staging deployment passes migration, health, rollback and security gates. |
...
| RQ-026 | PARTIAL | 388-test suite, migration/111-table restore target, Ruff/mypy/Bandit/audit/SBOM/secrets, locked frontend build, smoke and Playwright/Axe. | PR-head run `32297293629` passes all 388 tests, 111 restore checks, 117/117 mypy ratchet, zero-error 39-file slice and every configured gate. | Load/performance, manual assistive-tech, container/deployment and broader chaos tests are absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity for production-grade load tests. | Add performance-budget and non-root container validation gates. | Required functional, security, performance, recovery and deployment suites pass without waived critical findings. |
| RQ-027 | PARTIAL | Canonical roadmap, governance/runbooks and fail-closed review-only promotion. | CI/evidence records are traceable through Cycle 218; live pilot remains rejected. | No scheduled reports/cost service, executed external ops, production promotion or support model. | RQ-002, RQ-015, RQ-020, RQ-023–RQ-026. | Provider/cloud/broker approvals, budget, named operators and separate live authorization. | Add scheduled governance-report and cost-accounting contracts. | Operational reports, costs, runbooks and staged paper promotion are owner-approved and exercised; live requires a new explicit decision. |
...
2. **RQ-024/RQ-026 security and deployment quality:** external-session and
   auditable claim-mapping boundaries, nonce-based CSP, performance budgets and
   non-root container checks; retain manual assistive-technology acceptance as
   a deployment gate.
