...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0028`, research/risk/operations authorities; PostgreSQL integration/restore tests | Cycle 216 PR-head run `32293960662` verifies migration 0028, immutable retention policy/manifest/evaluation evidence and all 111 restored tables. Legacy research SQLite, actual object storage/queues and production deployment remain incomplete. |
| RQ-024 | Platform §23: security | PARTIAL | `security.py`, `config.py`, `audit.py`, API/dashboard header configuration, `.github/workflows/verify.yml`; API/config/role/browser tests | Cycle 218 PR run `32297293629` verifies viewer-by-default deployment composition and explicit viewer/researcher/data-steward/risk-reviewer/auditor/operator endpoint permissions with separate 401/403/503 failure semantics. This static one-token role is not production identity. OIDC, managed RBAC/MFA/sessions/CSRF, durable authorization audit, strict nonce-based dashboard CSP, secret manager/KMS, SSRF hardening, encrypted off-site backups, penetration evidence and incident operations remain incomplete. Never place secrets in frontend or source. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `Dockerfile`, `.github/workflows/verify.yml`, dashboard smoke and Playwright/Axe | Cycle 221 PR-head run `32302388728` passes all 401 tests without skips, all 111 restore checks, signed archive provenance/SBOM generation and verification, plus every existing gate. The full report retains 26 vendor-unfixed HIGH/CRITICAL findings and zero fixable ones. Load/performance, manual assistive-tech, registry-native OCI signing and production deployment testing remain incomplete. |
| RQ-027 | Platform §§29–35: phased operations, reports, cost governance, runbooks, live promotion | PARTIAL | `docs/*.md`, `operational_alerts.py`, `backup_recovery.py`, `strategy_promotion.py` | The roadmap and runbooks are traceable through Cycle 221 and distinguish verified provider-neutral evidence from external activation blockers; promotion is fail-closed to `REVIEW_REQUIRED`, not live. Scheduled operational reports/cost service, external provider runbooks executed under authorization, and production operations remain incomplete. Live pilot remains `REJECTED_WITH_REASON`: live execution is prohibited pending separate explicit authorization. |
...
| RQ-023 | PARTIAL | 28 Alembic migrations and 111-table restore allow-list; immutable job/run/local-outbox plus retention policy/manifest/evaluation evidence; SQLite retained for local research. | PR-head run `32293960662` proves migration 0028, rollback/restart/immutability and all 111 restore-critical tables. | No queue/object-store byte authority, lifecycle executor, deployed scheduler/backup topology; some research stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners for production. | Add deployment-owned scheduling/external delivery while preserving the separate no-delete boundary. | Production authorities, lifecycle/retention, backups and restore are migration-managed and tested. |
| RQ-024 | PARTIAL | Development bearer auth, explicit static role permissions, signed evidence, immutable audit, deterministic API/dashboard headers, production docs suppression and CI security/dependency/SBOM/secret gates. | Cycle 218 run `32297293629` proves viewer default, six-role least privilege, endpoint mapping and fail-closed 401/403/503 semantics on top of Cycle 217 HTTP/Axe gates. | No OIDC/managed RBAC/MFA, hardened sessions/CSRF, durable authorization-decision audit, nonce-based dashboard CSP, managed secrets/KMS or penetration evidence. | RQ-002, RQ-022–RQ-027. | Identity/cloud vendor decisions for final integration. | Add an external-session adapter contract and auditable claim-to-permission mapping without weakening viewer default. | Threat model and production identity/secrets controls pass independent security acceptance. |
...
2. **RQ-024/RQ-026 security and deployment quality:** external-session and
   auditable claim-mapping boundaries, nonce-based CSP and measured performance
   budgets; retain manual assistive-technology acceptance as a deployment gate.
