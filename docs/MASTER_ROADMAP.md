...
| RQ-011 | Platform §§10,15: probabilistic regimes, ensembles, portfolio construction | PARTIAL | `regimes.py`, `regime_engine_v2.py`, `portfolio_risk.py`, migration `0018`; regime/portfolio tests | Cycle 204 adds a feature/dataset/Data-Health-bound multidimensional regime authority with probabilities and uncertainty for every section 10 state, point-in-time/prefix-invariant reads, immutable OOS labels, Brier/accuracy comparison of rule-based and transparent ensemble methods, and non-executing risk-adjustment candidates that block automatic increases. HMM/change-point/clustering/Bayesian/tree methods remain explicitly unavailable, and the fixture is not empirical regime skill. PR CI run `31921339310` passed the PostgreSQL and restore gates; exact-merge verification remains pending. Correlation-aware portfolio optimisation and complete target-weight construction remain Cycle 205 scope. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0018`, Trend/Feature/Data Health/scorecard/validation/investment/regime authorities; PostgreSQL integration/restore tests | Migration 0018 adds content-addressed regime models/runs, reusable observations/evaluations with immutable run memberships, Data Health bindings and review-only risk candidates. PR CI run `31921339310` hash-matched all 62 restored critical tables; exact-merge verification remains pending. Legacy SQLite composition, object storage/queues, retention and production deployment remain incomplete. |
...
## Cycle 204 evidence — Regime Engine V2 (PR-verified)
