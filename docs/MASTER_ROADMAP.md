...
| RQ-017 | Platform §§13.7,21: auditable AI research agents and natural-language interface | PARTIAL | `agent_research.py`, `research_retrieval.py`, `api.py`, migration `0031`; agent/retrieval/API tests | The non-executing multi-role ledger, structured untrusted-output validation, contradiction/safety/human-review gates and read-only query API now have Cycle 225 point-in-time internal retrieval evidence. Pre-approved policies constrain source kinds, role access, availability/invalidation, result count, source diversity and query coverage; only `COMPLETE` immutable reports can authorize exact citation references for an agent request. Retrieval remains deterministic lexical fixture evidence with no model, connector, tool or action authority. No live model call, semantic/vector service, external ingestion, rich NL UX, scheduling or external-model truthfulness/safety evaluation exists. |
...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0031`, research/risk/operations/security/model/retrieval authorities; PostgreSQL integration/restore tests | Cycle 225 run `32551034230` verifies immutable retrieval policy/chunk/report/result evidence and all 120 restored tables. Legacy research SQLite, actual object storage/queues and production deployment remain incomplete. |
...
| RQ-017 | PARTIAL | Non-executing multi-role ledger plus immutable PIT/access-scoped lexical retrieval, structured adapter, safety/human review and read-only APIs. | Run `32551034230` proves filtering, insufficiency, exact citation binding, restart and immutability. | No semantic/vector retrieval, external ingestion, scheduler, rich UX or external-model truthfulness/safety evaluation. | RQ-004–RQ-008, RQ-022–RQ-025. | Model/source terms and credentials for actual calls. | Add agent answer-quality and scheduled-workflow evidence without tools or execution context. | Auditable retrieval/agent workflows meet provenance, safety, evaluation and human-review gates. |
...
| RQ-023 | PARTIAL | 31 Alembic migrations and 120-table restore allow-list; immutable operations, retention, identity, model-evaluation and research-retrieval evidence. | Run `32551034230` proves migration 0031 plus retrieval restart/immutability and all 120 tables. | No queue/object-store byte authority, deployed scheduler/backup topology; some stores remain SQLite. | RQ-002, RQ-024–RQ-027. | Managed services and retention policy owners. | Add deployment-owned scheduling/external delivery while preserving no-delete. | Production authorities and restore are migration-managed and tested. |
...
| RQ-026 | PARTIAL | 423-test suite, migration/120-table restore target and the existing full quality/security/supply-chain/frontend/browser gates. | Run `32551034230` passes 423 tests, 120 restore checks, 117/117 ratchet and zero-error 42-file slice. | Load/performance, manual assistive-tech, registry-native signing and production deployment remain absent. | RQ-002, RQ-022–RQ-025. | Representative staging capacity. | Add measured performance budgets when Chrome DevTools MCP is available. | Required suites pass without waived fixable critical findings. |
| RQ-027 | PARTIAL | Canonical roadmap, governance/runbooks and fail-closed review-only promotion. | Evidence is traceable through Cycle 225; live pilot remains rejected. | No scheduled reports/cost service, executed external ops, production promotion or support model. | RQ-002, RQ-015, RQ-020, RQ-023–RQ-026. | External approvals and named operators. | Add scheduled governance-report and cost-accounting contracts. | Operations are approved/exercised; live requires a new explicit decision. |
...
3. **RQ-016/RQ-017 model/agent evaluation:** extend Cycle 224 with explanation-
   sensitivity/degradation evidence and Cycle 225 with answer-quality evidence;
   keep both non-executing and fixture-labelled.
