...
| RQ-023 | Platform §22: normalized auditable database domain model | PARTIAL | `postgres_schema.py`, migrations `0001`–`0019`, Trend/Feature/Data Health/scorecard/validation/investment/regime/portfolio authorities; PostgreSQL integration/restore tests | Cycle 204 mainline run `31921534291` hash-matched 62 restored critical tables. Migration 0019 locally adds nine immutable portfolio policy/run/input/covariance/target/constraint/risk-gate tables and expands the restore set to 71; hosted verification is pending. Legacy SQLite composition, object storage/queues, retention and production deployment remain incomplete. |
...
| RQ-026 | Platform §§26–27: test strategy, CI/CD, performance/security/failure/restore tests | PARTIAL | Python tests, PostgreSQL integration/restore drill, `.github/workflows/verify.yml`, dashboard smoke | Mainline CI run `31921534291` on merge commit `cb3a38c` applied migration 0018, ran 328 tests without skips, matched 62 restored critical tables, and passed Ruff, the 120/120 mypy debt ratchet, a zero-error 23-file critical slice, Bandit, dependency audits, SBOM, secret scan, TypeScript, ESLint, Next build and dashboard smoke. Cycle 205 locally passes 333 tests with 30 PostgreSQL-only skips, a 24-file critical slice and migration head 0019; hosted evidence is pending. |
...
## Cycle 205 evidence — Multi-Strategy Portfolio Construction (in progress)
...
- Local verification discovered **333 tests**, passed all runnable tests with
  ...
  **24-file** critical slice and the sole Alembic head 0019. Hosted PostgreSQL
