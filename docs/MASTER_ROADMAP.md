...
| RQ-013 | Platform §§11.1–11.2: high-fidelity event simulation | PARTIAL | `event_backtest.py`, `paper_execution.py`, `corporate_actions.py`; `test_event_backtest.py`, `test_paper_execution.py`, `test_corporate_actions.py` | Deterministic event engine is exercised for latency, sessions, bid/ask, market/limit/stop/stop-limit, partial fills, fee, square-root impact, cash/position accounting and effective one-time corporate-action application. Funding/borrow/margin remain separate helpers; no futures/FX/tax/queue model or persistent backtest artifact exists. |
| RQ-014 | Upstream §36.10: independent cross-engine validation | PARTIAL | `cross_engine.py`, `research.py`, `strategy_promotion.py`; `test_cross_engine.py`, `test_research.py`, `test_strategy_promotion.py` | Final-equity/fill/position and fill identity/timestamp/quantity/price/fee mismatch tests pass. Every newly launched vectorized baseline experiment now compares final equity and turnover against a separate iterative close-to-close accounting implementation and persists the reconciliation result/differences. A research-only promotion gate blocks failed reconciliation. No identical-data three-engine strategy run, event-order reconciliation artifact, or corporate-action comparison. Connect vector and event orders with golden datasets. |
